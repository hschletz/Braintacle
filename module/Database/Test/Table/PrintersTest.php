...
class PrintersTest extends AbstractTest
{
    ...
    public function testHydrator()
    {
        ...
        $this->assertEquals('Driver', $map->hydrate('driver'));
        $this->assertEquals('Port', $map->hydrate('port'));
        $this->assertEquals('Description', $map->hydrate('description'));

        $this->assertEquals('name', $map->extract('Name'));
        $this->assertEquals('driver', $map->extract('Driver'));
        $this->assertEquals('port', $map->extract('Port'));
