...
# Which DBD driver will be used?
DB_SERVER_TYPE="Pg"
# Which MDB2 driver will be used?
MDB2_DRIVER="pgsql"
# Which host run database server
DB_SERVER_HOST="/var/run/postgresql"
...
ADM_SERVER_STATIC_DIR="/usr/local/share"
...
# Ask user for database server type
res=0
while [ $res -eq 0 ]
do
    echo -n "Which DBD driver will be used for the database server [$DB_SERVER_TYPE] ?"
    read ligne
    if [ -z "$ligne" ]
    then
	res=1
    else
	DB_SERVER_TYPE="$ligne"
	res=1
    fi
done
...
then
    ...
    echo "Checking for DBD::$DB_SERVER_TYPE PERL module..."
    echo "Checking for $DB_SERVER_TYPE PERL module" >> $SETUP_LOG
    $PERL_BIN -mDBD::$DB_SERVER_TYPE -e "print \"PERL module DBD::$DB_SERVER_TYPE is available\n\"" >> $SETUP_LOG 2>&1
    if [ $? -ne 0 ]
    then
...
then
    ...
    echo "Checking for DBD::$DB_SERVER_TYPE PERL module..."
    echo "Checking for DBD::$DB_SERVER_TYPE PERL module" >> $SETUP_LOG
    $PERL_BIN -mDBD::$DB_SERVER_TYPE -e "print \"PERL module DBD::$DB_SERVER_TYPE is available\n\"" >> $SETUP_LOG 2>&1
    if [ $? -ne 0 ]
    then
        echo "*** ERROR: PERL module DBD::$DB_SERVER_TYPE is not installed !"
    ...
    while [ $res -eq 0 ]
    do
	echo -n "Which MDB2 driver will be used for the database server [$MDB2_DRIVER] ?"
	read ligne
	if [ -z "$ligne" ]
	then
	    res=1
    ...
    echo "OK, MDB2 driver will be $MDB2_DRIVER ;-)"
    echo "MDB2 driver is $MDB2_DRIVER" >> $SETUP_LOG
    echo
    ...
    echo -n '$_SESSION["MDB2_DRIVER"]="' >> $ADM_SERVER_STATIC_DIR/$ADM_SERVER_STATIC_REPORTS_DIR/dbconfig.inc.php
    echo -n "$MDB2_DRIVER" >> $ADM_SERVER_STATIC_DIR/$ADM_SERVER_STATIC_REPORTS_DIR/dbconfig.inc.php
    ...
    $PERL_BIN -pi -e "s#OCS_DB_TYPE#$DB_SERVER_TYPE#g" ipdiscover-util.pl.local
    $PERL_BIN -pi -e "s#/var/run/postgresql#$DB_SERVER_HOST#g" ipdiscover-util.pl.local
    $PERL_BIN -pi -e "s#5432#$DB_SERVER_PORT#g" ipdiscover-util.pl.local
