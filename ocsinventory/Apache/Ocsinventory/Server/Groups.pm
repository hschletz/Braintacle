...
sub _validate_groups_cache{
  ...
  my $request = $dbh->prepare('
    ...
    WHERE ' . compose_unix_timestamp('CURRENT_TIMESTAMP') . '-REVALIDATE_FROM > ?
  ...
  while(my $row = $request->fetchrow_hashref()){
    ...
    if( !&_lock($row->{'HARDWARE_ID'}) ){
      ...
      my $check_request = $dbh->prepare('SELECT HARDWARE_ID, (' . compose_unix_timestamp('CURRENT_TIMESTAMP') . '-REVALIDATE_FROM) AS OFF FROM groups WHERE ' . compose_unix_timestamp('CURRENT_TIMESTAMP') . '-REVALIDATE_FROM > ? AND HARDWARE_ID=?');
...
# Updating cache time
  $dbh->do("UPDATE groups SET CREATE_TIME=" . compose_unix_timestamp('CURRENT_TIMESTAMP') . ", REVALIDATE_FROM=" . compose_unix_timestamp('CURRENT_TIMESTAMP') . "+? WHERE HARDWARE_ID=?", {}, $offset, $group_id);
