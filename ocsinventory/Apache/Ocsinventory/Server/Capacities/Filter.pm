...
# Clear cache
  $dbh->do( 'DELETE FROM conntrack WHERE (' . compose_unix_timestamp('CURRENT_TIMESTAMP') . '-' . compose_unix_timestamp('TIMESTAMP') . ')>?', {}, $flushEverySeconds );
  
# If we cannot insert ipadress, we consider that it is in cache, then forbid transmission
  if( !($current_context->{EXIST_FL}) && !( $dbh->do('INSERT INTO conntrack(IP,TIMESTAMP) VALUES(?,CURRENT_TIMESTAMP)', {}, $current_context->{IPADDRESS})) ){
