...
sub get_history_events {
  ...
  my $sth = get_sth( "SELECT DATE,DELETED,EQUIVALENT FROM deleted_equiv ORDER BY DATE,DELETED LIMIT $ENV{OCS_OPT_WEB_SERVICE_RESULTS_LIMIT} OFFSET $offset" );
...
sub clear_history_events {
  ...
  my $sth = get_sth( "SELECT * FROM deleted_equiv ORDER BY DATE,DELETED LIMIT $offset" );
  while( my $row = $sth->fetchrow_hashref() ) {
