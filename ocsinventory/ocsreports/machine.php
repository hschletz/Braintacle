...
if( isset( $_GET["suppack"] ) &  $_SESSION["lvluser"]==SADMIN  ) {
	if( $_SESSION["justAdded"] == false )
		@mdb2_query("DELETE FROM devices WHERE ivalue=? AND hardware_id=$systemid AND name='DOWNLOAD'", $_SESSION["writeServer"], "integer", $_GET["suppack"]);
...
if( isset( $_GET["actgrp"] )) {	
		...
		$valGroups = mdb2_fetch_assoc( $resGroups ); 
		if (isset($valGroups['id'])){
			$reqDelete = "DELETE FROM groups_cache WHERE hardware_id=$systemid AND group_id=?";
			
			if( $_GET["actgrp"] == 0 ) 
				$reqDelete .= " AND static<>0";
			$reqInsert = "INSERT INTO groups_cache(hardware_id, group_id, static) VALUES ($systemid,?,?)";
			@mdb2_query( $reqDelete, $_SESSION["writeServer"], "integer", $_GET["grp"]);
			if( $_GET["actgrp"] != 0 )
				@mdb2_query( $reqInsert, $_SESSION["writeServer"], array ("integer", "integer"), array ($_GET["grp"], $_GET["actgrp"]));
		}
}

$queryMachine    = "SELECT * FROM hardware WHERE (ID=$systemid)";
$result   = mdb2_query( $queryMachine, $_SESSION["readServer"] ) or mdb2_error($_SESSION["readServer"]);
$item     = mdb2_fetch_object($result, CASE_UPPER);
...
if ($_SESSION["MDB2_DRIVER"] == "mysql")
        $datatype = "SIGNED";
else
        $datatype = "INTEGER";
$sqlMem = "SELECT SUM(CAST(capacity AS $datatype)) AS capa FROM memories WHERE hardware_id=$systemid AND capacity !='No'";
$resMem = mdb2_query($sqlMem, $_SESSION["readServer"]) or die(mdb2_error($_SESSION["readServer"]));
$valMem = mdb2_fetch_assoc( $resMem );
...
if(!isset($_GET["tout"]))
		echo"<td width=50%><a style=\"text-decoration:underline\" href=\"machine.php?systemid=".urlencode($systemid)."&tout=1\"><img width='60px' src='image/ttaff.png' title='".$l->g(215)."'></a></td>";
...
function print_perso($systemid) {
	...
	$resultDetails = mdb2_query($queryDetails, $_SESSION["readServer"]) or die(mdb2_error($_SESSION["readServer"]));
					
		echo "<table BORDER='0' WIDTH = '95%' ALIGN = 'Center' CELLPADDING='0' BGCOLOR='#C7D9F5' BORDERCOLOR='#9894B5'>";
	
	while($item=mdb2_fetch_assoc($resultDetails, CASE_UPPER)) {
...
//		echo "$td3<a href='index.php?multi=22&systemid=$systemid&update=DOWNLOAD_SWITCH''>".$l->g(115)."</a></td>";		
	...
	$resGroups = @mdb2_query($sql_groups, $_SESSION["readServer"]) or die(mdb2_error($_SESSION["readServer"])); 
	
	if( mdb2_num_rows( $resGroups )>0 ) {
	...
	//TELEDEPLOY
	$resDeploy = @mdb2_query("SELECT a.name, d.tvalue,d.ivalue,d.comments, e.pack_loc  FROM devices d, download_enable e LEFT JOIN download_available a 
	ON e.fileid=a.fileid WHERE d.name='DOWNLOAD' AND e.id=d.ivalue AND d.hardware_id=$systemid"); 
	 
	if( mdb2_num_rows( $resDeploy )>0 ) {
			
		while( $valDeploy = mdb2_fetch_assoc( $resDeploy ) ) {
			$ii++; $td3 = $ii%2==0?$td2:$td4;
			echo "<tr>";
	...
	if (check_param ($_POST, "modification_param"))
...
function print_proc($systemid)
{
	...
	$resultDetails = mdb2_query($queryDetails, $_SESSION["readServer"]) or mdb2_error($_SESSION["readServer"]);
	$item = mdb2_fetch_object($resultDetails, CASE_UPPER);
	echo "<table BORDER='0' WIDTH = '95%' ALIGN = 'Center' CELLPADDING='0' BGCOLOR='#C7D9F5' BORDERCOLOR='#9894B5'>";
	echo "<tr>";
...
{	
	...
	$queryDetails  = "SELECT * FROM softwares WHERE (hardware_id=$systemid) ORDER BY publisher,name,version";
	$resultDetails = mdb2_query($queryDetails, $_SESSION["readServer"]) or die(mdb2_error($_SESSION["readServer"]));
...
{	
	...
	$queryDetails  = "SELECT h.pkg_id, a.name FROM download_history h LEFT JOIN download_available a ON CAST(h.pkg_id AS CHAR(255))=a.fileid WHERE h.hardware_id=$systemid ORDER BY pkg_id DESC";
	$resultDetails = mdb2_query($queryDetails, $_SESSION["readServer"]) or die(mdb2_error($_SESSION["readServer"]));
	...
	$ii = 0;
	while($item = mdb2_fetch_object($resultDetails))
	{	$ii++; $td3 = $ii%2==0?$td2:$td4;	
		echo "<tr>";
...
{	
	...
	while($item = mdb2_fetch_object($resultDetails, CASE_UPPER)) {	
		...
		blacklist("select ID from blacklist_macaddresses where macaddress=" . $_SESSION["readServer"]->quote (strtoupper (textDecode ($item->MACADDR)), "text"), textDecode($item->MACADDR),$l->g(704)." ".$l->g(708),$l->g(705)." ".$l->g(708),"Network(s)");
		echo "</td>";
...
{	
	...
	blacklist("select ID from blacklist_serials where SERIAL=" . $_SESSION["readServer"]->quote (textDecode ($item->SSN), "text"), textDecode($item->SSN),$l->g(704)." ".$l->g(707),$l->g(705)." ".$l->g(707),"BIOS");
//	if ($_SESSION["lvluser"]==SADMIN){
//		$sql_verif_blacklist="select ID from blacklist_serials where SERIAL='".textDecode($item->SSN)."'";
//		$result_blacklist = mysql_query($sql_verif_blacklist, $_SESSION["readServer"]) or die(mdb2_error($_SESSION["readServer"]));
...
function print_inventory($systemid)
{
	...
	$resultDetails = mdb2_query($queryDetails, $_SESSION["readServer"]) or die(mdb2_error($_SESSION["readServer"]));
	$item=mdb2_fetch_assoc($resultDetails);
	if( $item ) {			
		$label_bouton = "<input onmouseover=\"this.style.background='#FFFFFF';\" onmouseout=\"this.style.background='#C7D9F5'\" class='bouton' type='button' value='".$l->g(103)."' onClick='MAJ_donnees(\"$systemid\",\"".session_id()."\");'>";
...
function getNetName($did) {
	...
	$reqSub = "SELECT name FROM subnet s,networks n WHERE s.netid=n.ipsubnet AND n.hardware_id=?";
	$resSub = mdb2_query($reqSub, $_SESSION["readServer"], "integer", $did) or die(mdb2_error($_SESSION["readServer"]));
	$indice = 1;
	$returnVal = "<tr>".$tdhd.$l->g(304)." ".$indice.$tdhf;	

	while($valSub = mdb2_fetch_assoc( $resSub )){
		if($indice != 1) {
	...
	$queryDetails  = "SELECT ipsubnet FROM networks WHERE hardware_id=? AND ipsubnet NOT IN(SELECT netid FROM subnet)";
	$resultDetails = mdb2_query($queryDetails, $_SESSION["readServer"], "integer", $did) or die(mdb2_error($_SESSION["readServer"]));	
	while($item = mdb2_fetch_assoc($resultDetails)) {
		if($indice != 1) {
...
function isAvail($lbl) {
	...
	$resAv = mdb2_query("SELECT hardware_id FROM $tble WHERE hardware_id=$systemid", $_SESSION["readServer"] );
	//echo "SELECT hardware_id FROM $tble WHERE hardware_id=$systemid";
	$valAvail = mdb2_num_rows( $resAv );
	return ($valAvail>0);
}
...
function blacklist($sql_verif_blacklist,$serial_mac,$lblpopup_blacklist,$lblpopup_unblacklist,$direct){
	...
	<?php
	if ($_SESSION["lvluser"]==SADMIN){
		$result_blacklist = mdb2_query($sql_verif_blacklist, $_SESSION["readServer"]) or die(mdb2_error($_SESSION["readServer"]));
		if ( mdb2_num_rows($result_blacklist) == 0 )
		//<a href='./machine.php?systemid=".$systemid."&option=BIOS&black=".textDecode($item->SSN)."'>
		
	   echo " <a href=# OnClick='confirme(\"".$serial_mac."\",\"black\",\"".$lblpopup_blacklist."\");' title='".$l->g(703)."'><img height=10 src=image/interdit.jpg></a>";
	   else{
	   	$item_blacklist = mdb2_fetch_object($result_blacklist, CASE_UPPER);	
	   	// <a href='./machine.php?systemid=".$systemid."&option=BIOS&noblack=".$item_blacklist->ID."'>   		
...
function update_blacklist(){
	// blacklist serial
	if (isset($_GET['black']) &  $_SESSION["lvluser"]==SADMIN && check_param ($_GET, "option") == "BIOS")
		@mdb2_query("INSERT INTO blacklist_serials (SERIAL) VALUES (?)", $_SESSION["writeServer"], "text", $_GET['black']);

	//blacklist mac
	if (isset($_GET['black']) &  $_SESSION["lvluser"]==SADMIN && check_param ($_GET, "option") == "Network(s)")
		@mdb2_query("INSERT INTO blacklist_macaddresses (MACADDRESS) VALUES (?)", $_SESSION["writeServer"], "text", strtoupper ($_GET['black']));
		
	// unblacklist serial
	if (isset($_GET['noblack']) &  $_SESSION["lvluser"]==SADMIN && check_param ($_GET, "option") == "BIOS")	
		@mdb2_query("DELETE FROM blacklist_serials WHERE id=?", $_SESSION["writeServer"], "integer", $_GET['noblack']);
		
	// unblacklist mac 
	if (isset($_GET['noblack']) &  $_SESSION["lvluser"]==SADMIN && check_param ($_GET, "option") == "Network(s)")	
		@mdb2_query("DELETE FROM blacklist_macaddresses WHERE id=?", $_SESSION["writeServer"], "integer", $_GET['noblack']);
