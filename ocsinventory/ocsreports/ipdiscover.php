...
$sql_IPD_DIR="select tvalue from config where NAME='IPDISCOVER_IPD_DIR'";
	$res_IPD_DIR = mdb2_query( $sql_IPD_DIR, $_SESSION["readServer"] );
	while( $val_IPD_DIR = mdb2_fetch_assoc( $res_IPD_DIR ) ) {
...
$_SESSION["pas"] = isset($_GET["pas"]) ? $_GET["pas"] : check_param ($_SESSION, "pas");
$nomRez = getNameFromRes($_SESSION["pas"]);

switch( check_param ($_GET, "mode") ) {
	case 12: $_SESSION["retour"] = "multi=3&mode=12"; break;
	case 6:  $_SESSION["retour"] = "multi=3&mode=6&pas=".$_GET["pas"]."&nam=".$_GET["nam"]; break;
	case 2:  $_SESSION["retour"] = "multi=3&mode=2&pas=".$_GET["pas"]."&nam=".check_param ($_GET, "nam"); 
			if( isset($filePresent) and $filePresent) {
				if( ! is_executable($fipdisc) ) {
...
if( isset($_GET["delmac"]) ) {
	mdb2_query("DELETE FROM netmap WHERE mac=?", $_SESSION["writeServer"], "text", strtoupper ($_GET["delmac"])) or die(mdb2_error());
}
	
if( ! $scriptPresent && ! check_param ($_GET, "mode") ) {
	$scriptPresent = false ;
}
...
<?php 	
	...
	if( isset( $_POST["subRez"] ) ) {
		if( $_POST["nomrez"] == "" || $_POST["dpt"] == "" || $_POST["ipa"] == "" || $_POST["ipm"] == "" ) {
			...
			$reqDelete = "DELETE FROM subnet WHERE netid = ?";
			@mdb2_query( $reqDelete, $_SESSION["writeServer"], "text", $_POST["ipa"]);
			$reqInsert = "INSERT INTO subnet(name, id, netid, mask) VALUES (?,?,?,?)";
			@mdb2_query( $reqInsert, $_SESSION["writeServer"], array ("text", "integer", "text", "text"), array ($_POST["nomrez"], $_POST["dpt"], $_POST["ipa"], $_POST["ipm"]));
			if(mdb2_affected_rows()>0)
				echo "<br><center><font color='green'><b>".$l->g(301)." (".htmlentities($_POST["nomrez"])."  ".$_POST["dpt"]."  ".$_POST["ipa"]." / ".$_POST["ipm"].")</b></font></center>";
			else
	...
	if( isset($_GET["delrez"])) {
		unset($_SESSION["lastTri"]);
		$reqSupp = "DELETE FROM subnet WHERE netid=?";
		mdb2_query( $reqSupp, $_SESSION["writeServer"], "text", $_GET["delrez"]) or die(mdb2_error($_SESSION["writeServer"]));
	...
	$resSubnet = mdb2_query($reqSubnet, $_SESSION["readServer"]) or die(mdb2_error($_SESSION["readServer"]));
	
	while( $valSubnet = mdb2_fetch_assoc($resSubnet) ) {
		$tab[] = array( "<a href=index.php?multi=3&self=1&mode=11&ipa=".urlencode($valSubnet["netid"])."&nomrez=".urlencode($valSubnet["name"])."&dpt=".urlencode($valSubnet["id"])."&ipm=".urlencode ($valSubnet["mask"]).">" . htmlspecialchars ($valSubnet["name"]) . "</a>",
			$valSubnet["id"],$valSubnet["netid"],  $valSubnet["mask"] ,
		"<a href='index.php?multi=3&mode=11&tri=".check_param ($_GET, "tri")."&delrez=".urlencode($valSubnet["netid"])."'><img src=image/supp.png></a>");
		$exist = true;
	}
	
	printEnTete($l->g(303));
	$tri = isset($_GET["tri"])?$_GET["tri"]:check_param ($_POST, "tri");
	...
	<table align='center'>
		<tr><td><?php echo $l->g(304);?> :</td><td><input type='text' size='50' name='nomrez' value="<?php echo htmlspecialchars ($ValNomRez);?>"></td><td>&nbsp;&nbsp;
		<?php echo $l->g(305);?> :</td><td><input type='text' size='3' name='dpt' value='<?php echo htmlspecialchars ($ValDpt);?>'></td></tr>
		</tr><tr><td><?php echo $l->g(34);?> :</td><td><input type='text' name='ipa' value='<?php echo htmlspecialchars ($ValIpa);?>'</td><td>&nbsp;&nbsp;<?php echo $l->g(208);?>:
		</td><td><input type='text' name='ipm' value='<?php echo htmlspecialchars ($ValIpm);?>'></td></tr>
	...
	if( isset( $_POST["subTyp"]) && $_POST["nomtyp"]!="" ) {
		unset($_SESSION["lastTri"]);
		$reqAddTyp = "INSERT INTO devicetype(name) VALUES (?)";
		mdb2_query( $reqAddTyp , $_SESSION["writeServer"], "text", $_POST["nomtyp"]) or die(mdb2_error($_SESSION["writeServer"]));
	}
	
	if( isset( $_GET["deltyp"] )) {
		$reqDelTyp = "DELETE FROM devicetype WHERE name=?";
		mdb2_query( $reqDelTyp, $_SESSION["writeServer"], "text", urldecode($_GET["deltyp"])) or die(mdb2_error($_SESSION["writeServer"]));
	}
	...
	$resType = mdb2_query( $reqTypes, $_SESSION["readServer"] ) or die(mdb2_error($_SESSION["readServer"]));
	$tab[0] = Array("Type","");
	$tailles = Array(200,30);
	$cptligne = 1;
	while( $valType = mdb2_fetch_assoc($resType) ) {
		$tab[] = Array( htmlspecialchars ($valType["name"]), "<a href='index.php?multi=3&mode=10&deltyp=".urlencode($valType["name"])."'><img src=image/supp.png></a>");
		$cptligne++;
		$exist = true;
	}
	if( isset ($exist) ) {
...
// Page with all detailed networks
	...
	$resIpConf = mdb2_query( $reqIpConf, $_SESSION["readServer"] ) or die(mdb2_error($_SESSION["readServer"]));
	$valIpConf = mdb2_fetch_assoc( $resIpConf );
	...
	$totNinvRes = mdb2_query( $totNinvReq, $_SESSION["readServer"]) or die(mdb2_error($_SESSION["readServer"]));
	$totNinvVal = mdb2_fetch_assoc( $totNinvRes );
	...
	if( isset($_GET["uid"]) && is_numeric($_GET["uid"]) ) {
		...
		$dpt = check_param ($_COOKIE, "DefNetwork");
	}
		
	if( !isset($dpt) || check_param ($_GET, "uid")==$l->g(215) || ( !isset($_GET["uid"]) && $_COOKIE["DefNetwork"]==$l->g(215)) ) {
		...
		$totNinvResLoc = mdb2_query( $totNinvReqLoc, $_SESSION["readServer"], "integer", $dpt) or die(mdb2_error($_SESSION["readServer"]));
		$totNinvValLoc = mdb2_fetch_assoc( $totNinvResLoc );
		
		echo "<center><b></b></center><br>";
		$reqGateway = "SELECT ipsubnet as nbrez, COUNT(hardware_id) AS nbc FROM networks n,subnet s
		WHERE ipsubnet<>'0.0.0.0' AND description NOT $LIKE '%PPP%' AND n.ipsubnet=s.netid AND s.id = " . $_SESSION["readServer"]->quote ($dpt, "integer") . " GROUP BY(ipsubnet) ";
	...
	echo "<td width='50%'><table width='100%' align='center'><tr><td align='center'><b>".$l->g(305).":</b></td></tr>
		...
		<option".(isset ($tout)?" selected":"").">".$l->g(215)."</option>";
	$reqDropDown = "SELECT DISTINCT(id) FROM subnet ORDER BY id";
	$resDropDown = mdb2_query( $reqDropDown, $_SESSION["readServer"] );
	while( $valDropDown = mdb2_fetch_assoc( $resDropDown ) ) {
		echo "<option".($valDropDown["id"]==$dpt?" selected":"")." value='".$valDropDown["id"]."'>".$valDropDown["id"]."</option>";
	}
	echo "</select></form></td></tr></table></td></tr>";	
	echo "</table>";
	$auMoinsUnRezo = false;
	$resGateway = mdb2_query($reqGateway, $_SESSION["readServer"]) or die(mdb2_error($_SESSION["readServer"]));	

	while( $arrGateway = mdb2_fetch_assoc($resGateway) ) {
		$auMoinsUnRezo = true;
		$resIpd = mdb2_query("SELECT COUNT(*) AS nbi FROM devices WHERE name='IPDISCOVER' AND tvalue=?", $_SESSION["readServer"], "text", $arrGateway["nbrez"]) or die(mdb2_error($_SESSION["readServer"]));
		$arrIpd = mdb2_fetch_assoc( $resIpd );
		...
		$reqSubnet = "SELECT name,id,mask FROM subnet WHERE netid=?";
		$resSubnet = mdb2_query($reqSubnet, $_SESSION["readServer"], "text", $arrGateway["nbrez"]) or die(mdb2_error());
		if( $valSubnet = mdb2_fetch_assoc( $resSubnet ) ) {			
			$t[ $cptL ][] = "";
		...
		$reqNonInv = "SELECT COUNT(*) AS nbnoninv FROM netmap WHERE NETID=? 
		AND mac NOT IN (SELECT DISTINCT(macaddr) FROM networks WHERE macaddr IS NOT NULL AND IPSUBNET=?) AND mac NOT IN (SELECT DISTINCT(macaddr) FROM network_devices)";
		
		$resNonInv = mdb2_query($reqNonInv, $_SESSION["readServer"], array("text", "text"), array($arrGateway["nbrez"], $arrGateway["nbrez"])) or die(mdb2_error());
		
		if( $valNonInv = mdb2_fetch_assoc( $resNonInv ) ) {	
		...
		$reqSais = "SELECT COUNT(id) as nbsais FROM network_devices n,netmap a WHERE a.netid=? AND n.macaddr=a.mac";
		
		$resSais = mdb2_query($reqSais, $_SESSION["readServer"], "text", $arrGateway["nbrez"]) or die(mdb2_error());
		if( ($valSais = mdb2_fetch_assoc( $resSais )) && ($valSais["nbsais"] > 0)) {
	...
	$strEnTete = $l->g(367)." ".$_GET["pas"]."<br><br>" . htmlspecialchars ($nomRez);
	printEnTete($strEnTete);
	...
	if($_GET["pas"] == ""){
		...
		$finSubnet = "=" . $_SESSION["readServer"]->quote ($_GET["pas"], "text");
	...
	if($_GET["pas"] == ""){
		...
		$finGateway = "=" . $_SESSION["readServer"]->quote ($_GET["pas"], "text");
	
	$lbl = $l->g(314)." ".$_GET["pas"];	
	$sql = "d.name='IPDISCOVER' AND (d.ivalue=1 OR d.ivalue=2) AND d.tvalue =" . $_SESSION["readServer"]->quote ($_GET["pas"], "text");
...
// Network analyze
	...
	printEnTete($l->g(316)." ".$_GET["pas"]."<br><br>" . htmlspecialchars ($nomRez));
?>
...
<?php 
	...
	$reqRez = "SELECT ip, mac, mask, date, name FROM netmap WHERE netid=".$_SESSION["readServer"]->quote($_GET["pas"], "text")." AND mac NOT IN (SELECT DISTINCT(macaddr) FROM networks WHERE IPSUBNET=".$_SESSION["readServer"]->quote($_GET["pas"], "text").") 
	AND mac NOT IN (SELECT DISTINCT(macaddr) FROM network_devices)";
	$resRez = mdb2_query( $reqRez, $_SESSION["readServer"] ) or die(mdb2_error());
	...
	$cptL = 1;
	while( $valRez = mdb2_fetch_assoc($resRez) )  {
...
//ANALYZE MODE
	...
	$rez = $nomRez;
	printEnTete($l->g(319)." $pas<br><br>" . htmlspecialchars ($rez));
...
<script language=javascript>document.getElementById(\"wait\").innerHTML=\"\";</script>";
	...
	if( check_param ($_GET, "direct") != 2 && $_SESSION["retour"] != "multi=3&mode=1" ) 
	...
	if( isset( $_POST["macaddr"] )) {
		...
		$reqIp = "SELECT ip,netid FROM netmap WHERE mac = ?";
		$resIp = mdb2_query($reqIp, $_SESSION["readServer"], "text", strtoupper ($_POST["macaddr"])) or die(mdb2_error($_SESSION["readServer"]));
		$valIp = mdb2_fetch_assoc( $resIp );
		unset($_SESSION["lastTri"]);
		if( checkNetwork($_POST["macaddr"]) ) {
			$reqNet = 'INSERT INTO network_devices(description, type, macaddr, ' . mdb2_quote_identifier ("user", $_SESSION["writeServer"]) .
			") VALUES (?,?,?,?)";
			mdb2_query($reqNet, $_SESSION["writeServer"], array("text", "text", "text", "text"), array ($_POST["description"], $_POST["type"], strtoupper ($_POST["macaddr"]), $_SESSION["loggeduser"])) or die(mdb2_error($_SESSION["writeServer"]));
		}
		else
	...
	$ent = $l->g(334)." ".$l->g(368)." ".$_GET["pas"]."<br><br>" . htmlspecialchars ($nomRez);

	printEnTete($ent);echo "<br>";	
	if( isset($_GET["pas"]) ) {
		$sql  = "a.netid=" . $_SESSION["readServer"]->quote ($_GET["pas"], "text");
	}
...
function trieTab($t,$colTri,$types) {
	...
	if( check_param ($_SESSION, "lastTri") == $colTri ) {
...
function printTab($t ,$modeReg=false, $tailles=null, $unSurDeux=false, $scroll = false, $modeDel = false) {
	...
	foreach($_GET as $nom=>$val) {
		...
		$lesget .= "&amp;" . urlencode ($nom) . "=" . urlencode($val);
	}
	...
	if( $scroll ) {
		...
		foreach($t[0] as $case) {
			...
			echo "$tdsp<a href=?tri=$col{$lesget}><b>" . htmlspecialchars ($case) . "</b></a>$ftd";
			$col++;
		}
		echo "</tr></table></div>";
	}
	...
	while( isset($t[$ligne]) ) { // chque ligne		
		...
		for( $col=0 ; $col < sizeof($t[$ligne]) ; $col++) {
			...
			if($ligne==0) {
				$taille = $tailles!=null ? $tailles[$col] : "200px" ;
				$tdsp = "<td align='center' width='{$taille}px'>";
				echo "$tdsp<a href=?tri=$col{$lesget}><b>" . htmlspecialchars ($c) . "</b></a>$ftd";
			}
			else {
				echo $td.$ah.$c.$ftd;
			}
		...
		if( $modeReg ) {
			if($ligne==0)
				...
				echo "<td align=center><a href=index.php?popup=1&multi=3&mode=8&direct=4&mac=" . urlencode ($t[$ligne][1]) . "&nam=" . urlencode ($t[$ligne][2]) . "&pas=" . urlencode($_GET["pas"]) . "><img src='image/Gest_admin1.png'></a></td>";
			else
				echo "<td align=center><a href=index.php?popup=1&multi=3&mode=8&direct=3&mac=" . urlencode ($t[$ligne][1]) . "&nam=" . urlencode ($t[$ligne][2]) . "&pas=" . urlencode($_GET["pas"]) . "><img src='image/Gest_admin1.png'></a></td>";
		...
		if( $modeDel ) {
			if($ligne==0)
				...
				echo "<td align=center><a href=index.php?popup=1&multi=3&mode=".$_GET["mode"]."&delmac=".urlencode ($t[$ligne][1]) . "&pas=" . urlencode($_GET["pas"]) . "><img src='image/supp.png'></a></td>";
		}
...
function runCommand($command="") {
	global $l,$IPD_DIR;
	$command = escapeshellcmd ("perl ipdiscover-util.pl $command -xml -h=".$_SESSION["SERVEUR_SQL"]." -u=".$_SESSION["COMPTE_BASE"]." -p=".$_SESSION["PSWD_BASE"]." -path=".$IPD_DIR."/ipd");
	//echo $command."<br>";
	$fd = popen($command,"r");	
	if($fd==FALSE) {
		echo "pas de handle";
...
function getMask( $ip ) {
	$reqMsk = "SELECT ipmask FROM networks WHERE ipsubnet=? AND ipmask <>''";
	$resMsk = mdb2_query( $reqMsk, $_SESSION["readServer"], "text", $ip) or die( mdb2_error($_SESSION["readServer"]) );
	while( $ligMsk = mdb2_fetch_row( $resMsk ) ) {
...
function getNameFromRes($nbrez) {
	if (is_null ($nbrez)) return "";
	$reqSub = "SELECT name FROM subnet s WHERE s.netid=?";
	$resSub = mdb2_query($reqSub, $_SESSION["readServer"], "text", $nbrez) or die(mdb2_error($_SESSION["readServer"]));
	if($valSub = mdb2_fetch_assoc( $resSub )){
		return $valSub["name"];
	}
	return "";
}
...
function checkNetwork($mac) {
	$reqMac = "SELECT id FROM network_devices WHERE macaddr=?";
	$resMac = mdb2_query( $reqMac, $_SESSION["readServer"], "text", strtoupper ($mac)) or die(mdb2_error($_SESSION["readServer"]));
	return( mdb2_num_rows( $resMac ) == 0 );
}
?>
