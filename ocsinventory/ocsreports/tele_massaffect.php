...
function affectPackage( $computer, $packageId ) {
	...
	$reqName = "SELECT id FROM hardware WHERE name=?";
	$resName = @mdb2_query( $reqName , $_SESSION["readServer"], "text", $computer);
	...
	@mdb2_query( "DELETE FROM devices WHERE name='DOWNLOAD' AND IVALUE=? AND hardware_id=?", $_SESSION["writeServer"], array ("integer", "integer"), array ($packageId, $computerId));
	
	if( ! @mdb2_query( "INSERT INTO devices(HARDWARE_ID, NAME, IVALUE) VALUES(?, 'DOWNLOAD', ?)", $_SESSION["writeServer"], array ("integer", "integer"), array ($computerId, $packageId) )) {
		return false;
	}	
...
<input type='hidden' name='id' value='<?php echo check_param ($_POST, "id")?$_POST["id"]:$_GET["id"]; ?>'>
</form>
