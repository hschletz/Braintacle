...
define ("allow_direct_invocation", true);
...
if( (!$fconf=@fopen("dbconfig.inc.php","r")) || (!function_exists('session_start')) || (!class_exists('MDB2'))) {
	require('install.php');
	die();
}
else
...
$pcparpage = check_param ($_SESSION, "pcparpage", "^[0-9]*$");

if( ! $pcparpage ) {
	if( isset($_COOKIE["PcParPage"]) )
		$pcparpage = check_param ($_COOKIE, "PcParPage", "^[0-9]*$");
	else	
...
if(isset($_GET["lareq"]))  {
	//unset( $_SESSION["c"] );
	$_SESSION["lareqpages"] = $_GET["lareq"];
}
...
else if(check_param ($_GET, "multi") != 6)
	$_SESSION["pageCur"] = 1;

if(check_param ($_GET, "supp") && check_param ($_GET, "multi") != 8) {
		deleteDid($_GET["supp"]);
}
if( check_param ($_GET, "suppnet") ) {
...
if(! isset($_SESSION["first"]) || !check_param ($_GET, "lareq")) {
	$_GET["lareq"] = $l->g(2);
	$_SESSION["first"] = true;
}
	$i=0;
	...
	$res = mdb2_query($sql_groups_4all, $_SESSION["readServer"]) or die(mdb2_error($_SESSION["readServer"]));
	$item = mdb2_fetch_object($res);
	if (isset($item->workgroup) or $_SESSION["lvluser"]==SADMIN or $_SESSION["lvluser"]==LADMIN)	
	echo "<a href=index.php?multi=37><img title=\"".htmlspecialchars($l->g(583))."\" src='image/".(check_param ($_GET, "multi")==37?"groups_a.png":"groups.png")."'></a>";
	
	//$countHl++;
	//multicrit
	echo "<a href=index.php?multi=36><img title=\"".htmlspecialchars($l->g(765))."\" src='image/".(check_param ($_GET, "multi")==36?"ttlogiciels_a.png":"ttlogiciels.png")."'></a>";
	//$countHl++;
	echo "<a href=index.php?multi=1><img title=\"".htmlspecialchars($l->g(9))."\" src='image/" . (check_param ($_GET, "multi") == 1 || (!isset($_GET["multi"]) && check_param ($_GET, "lareq") == $l->g(9)) ? "recherche_a.png":"recherche.png")."'></a>";
	...
	if(isset($_POST["subPass"])) {
		if(!$_POST["pass1"] || !$_POST["pass2"]) {
			...
			mdb2_query("UPDATE operators SET passwd=? WHERE id=?", $_SESSION["writeServer"], array ("text", "text"), array (md5 ($_POST["pass1"]), $_SESSION["loggeduser"]));
		}
	}	
...
function menu_list($name_menu,$packAct,$nam_img,$title,$data_list)
{
	...
	if( in_array (check_param ($_GET, "multi"), $packAct) )
		echo "_a";
		echo ".png'></a></dt>
...
function tab( $label, $multi, $lien=null ) {
	...
	if( !$showingReq && (check_param ($_GET, "multi") == $multi && $multi != "") ) {
		$img .= "_a";
	}
