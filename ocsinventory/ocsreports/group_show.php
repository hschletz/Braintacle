...
if( $_SESSION["lvluser"]!=LADMIN && $_SESSION["lvluser"]!=SADMIN  ){
	...
	$res_verif = mdb2_query($sql_verif, $_SESSION["readServer"]) or die(mdb2_error($_SESSION["readServer"]));
...
if( isset($_POST["actshowgroup"])) {
	foreach( $_POST as $key=>$val ) {
		if( strpos ( $key, "checkmass" ) !== false ) {
			...
			$resDelete = "DELETE FROM groups_cache WHERE hardware_id=? AND group_id=".$systemid;
			@mdb2_query( $resDelete, $_SESSION["writeServer"], "integer", $val);
			if( $_POST["actshowgroup"] != 0 ) {
				$reqInsert = "INSERT INTO groups_cache(hardware_id, group_id, static) VALUES (?,?,?)";
				$resInsert = mdb2_query( $reqInsert, $_SESSION["writeServer"], array ("integer", "integer", "integer"), array ($val, $systemid, $_POST["actshowgroup"]));
			}
		}
	}	
}
//update values if user want modify groups' values
if (check_param ($_POST, "Valid_modif_x") and !isset($_POST['modif']))
{
	if (trim($_POST['NAME'])!= '' and trim($_POST['DESCR'])!=''){
		...
		$result = mdb2_query($req, $_SESSION["writeServer"], array ("text", "text"), array ($_POST['NAME'], $_POST['DESCR'])) or die(mdb2_error($_SESSION["writeServer"]));
	}
	else{
...
function print_computers_real($systemid) {
	...
	$resGroup = mdb2_query( $reqGroup, $_SESSION["readServer"] ) or mdb2_error($_SESSION["readServer"]);
	$valGroup = mdb2_fetch_assoc($resGroup);
...
function print_computers_cached($systemid) {
	...
	$idList = getGluedIds($reqIds);
	if (!$idList)
		$idList = "NULL";
	$sql = " deviceid<>'_SYSTEMGROUP_' AND deviceid <> '_DOWNLOADGROUP_' AND h.id IN ($idList)";
...
function print_perso($systemid) {
	...
	//TELEDEPLOY
	$resDeploy = @mdb2_query("SELECT a.name, d.tvalue,d.ivalue, e.pack_loc,e.fileid  FROM devices d, download_enable e LEFT JOIN download_available a 
...
function show_stat($fileId){
	...
	echo $td3."<a href=\"tele_stats.php?stat=" . htmlspecialchars ($fileId) . "&group=".$_GET['systemid']."\" target=_blank><img src='image/stat.png'></a></td>";
}
