...
if ($_SESSION["MDB2_DRIVER"] == "pgsql") {
       // PostgreSQL knows a case-insensitive ILIKE operator which will be used instead of the standard LIKE operator.
       $LIKE = "ILIKE";
} else {
       // Use standard LIKE operator for other DBMS. Case-sensitivity depends on the implementation.
       $LIKE = "LIKE";
}

define("DEFAULT_LANGUAGE", "english");	// the only usable language. Other translations (except french) are incomplete and/or mistranslated
...
define("UTF8_DEGREE", 0 );				// 0 For non utf8 database, 1 for utf8. Set to 0 if Database handles charset conversion.
...
if( !in_array( check_param ($_GET, "multi"), array(20,21,26,22,24,27)) ) {
	$_SESSION["queryString"] = "";
	foreach( $_GET as $key=>$val )
		$_SESSION["queryString"] .= "&amp;" . urlencode ($key) . "=" . urlencode ($val);
}
...
if( ! isset($_SESSION["availFieldList"]) ) {
	...
	foreach ($tableInfo as $colname)
		$lesCols[] = Array( "pref"=>"a", "val"=>$colname["name"] );

	$tableInfo = $_SESSION["readServer"]->reverse->tableInfo("bios");
	if (PEAR::isError($tableInfo))
		die ($tableInfo->getMessage());
	foreach ($tableInfo as $colname)
		$lesCols[] = Array( "pref"=>"b", "val"=>$colname["name"] );

	$tableInfo = $_SESSION["readServer"]->reverse->tableInfo("hardware");
	if (PEAR::isError($tableInfo))
		die ($tableInfo->getMessage());
	foreach ($tableInfo as $colname)
		$lesCols[] = Array( "pref"=>"h", "val"=>$colname["name"] );
...
if(!isset($_SESSION["currentFieldList"])) {// gui just launched
	if( isset( $_COOKIE["col"] ) ) { // columns cookie set ?
		...
		foreach( $cookSort as $val ) { // loading ordered values
			$_SESSION["currentFieldList"][$val["name"]] = $val["value"];
		}
...
if(isset($_GET["suppCol"])) { // a column must be removed
	$keyName = array_case_search($_GET["suppCol"], $_SESSION["currentFieldList"] ); // look for the column's key
...
if(isset( $_SESSION["currentFieldList"] ) && $loadedFromCookie ) {
	...
	foreach ($tableInfo as $valAccount)
		$fieldsAvailable[] = strtoupper($valAccount["name"]);
...
function addComputersToGroup( $gName, $ids ) {
	...
	$reqIdGroup = "SELECT id FROM hardware WHERE name=?";
	$resIdGroup = mdb2_query( $reqIdGroup, $_SESSION["readServer"], "text", $gName);
	$valIdGroup = mdb2_fetch_assoc( $resIdGroup );
	...
	if( lock( $valIdGroup["id"] ) ) {
		...
		foreach( $ids as $key=>$val ) {
			if( strpos ( $key, "checkmass" ) !== false ) {
				$idsList[] = $val;
				$resDelete = "DELETE FROM groups_cache WHERE hardware_id=? AND group_id=?";
				@mdb2_query( $resDelete, $_SESSION["writeServer"], array ("integer", "integer"), array ($val, $valIdGroup["id"]) );
				
				$reqInsert = "INSERT INTO groups_cache(hardware_id, group_id, static) VALUES (?, ?, 1)";
				$resInsert = mdb2_query( $reqInsert, $_SESSION["writeServer"], array ("integer", "integer"), array ($val, $valIdGroup["id"]) );
				$nb_res++;
			}
		}
...
function createGroup( $name,$description="", $staticOnly=false, $alreadyExists = false ) {
	...
	$resGetId = mdb2_query( $reqGetId, $_SESSION["readServer"], "text", $name);
	...
	if( ! $staticOnly )
		$request = "SELECT DISTINCT h.id " . $_SESSION["groupReq"];
	else
		$request = "";
		
	mdb2_query( "INSERT INTO hardware(deviceid,name,description,lastdate) VALUES(?, ?, ?, CURRENT_TIMESTAMP )", $_SESSION["writeServer"], array ("text", "text", "text"), array ($deviceid, $name, $description) ) 
	...
	//Creating group
	mdb2_query( "INSERT INTO groups(hardware_id, request, create_time) VALUES ( $insertId, ?, " . $_SESSION["writeServer"]->function->unixtimestamp ("CURRENT_TIMESTAMP") . ")", $_SESSION["writeServer"], "clob", $request) 
	or die( mdb2_error($_SESSION["writeServer"]) );

	//Generating cache
	if( ! $staticOnly && lock($insertId) ) {	
		// pick already existing hardware_id/group_id tuples
		$existing = $_SESSION["readServer"]->queryCol ("SELECT hardware_id FROM groups_cache WHERE group_id=$insertId");
		if (PEAR::isError ($existing)) die ($existing->getMessage());

		// pick IDs to be inserted
		$to_be_inserted = $_SESSION["readServer"]->queryCol ("SELECT DISTINCT h.id " . $_SESSION["groupReq"]);
		if (PEAR::isError ($to_be_inserted)) die ($to_be_inserted->getMessage());

		// remove already existing IDs
		$to_be_inserted = array_diff ($to_be_inserted, $existing);

		// insert remaining IDs
		$cached = 0;
		foreach ($to_be_inserted as $hardware_id) {
			$result = $_SESSION["writeServer"]->exec ("INSERT INTO groups_cache (hardware_id, group_id, static) VALUES ($hardware_id, $insertId, 0)");
	...
	echo "<br><center>".$l->g(607)." <b>" . htmlspecialchars ($name) . "</b> " . (!$alreadyExists?$l->g(608):$l->g(609))." ".(isset($cached)?$l->g(622).":".$cached:"")."<br>";
	return true;
}

function dbconnect($encoding) {
	$db = DB_NAME;
	
	$link=@mdb2_connect($_SESSION["MDB2_DRIVER"],$_SESSION["SERVER_READ"],$_SESSION["COMPTE_BASE"],$_SESSION["PSWD_BASE"],$db,$encoding);
	if(!$link) {
		echo "<br><center><font color=red><b>ERROR: Database connection problem<br>".mdb2_error()."</b></font></center>";
		require('install.php');
		die();
	}
		
	$link2=@mdb2_connect($_SESSION["MDB2_DRIVER"],$_SESSION["SERVER_WRITE"],$_SESSION["COMPTE_BASE"],$_SESSION["PSWD_BASE"],$db,$encoding);
	if(!$link2) {
		echo "<br><center><font color=red><b>ERROR: Database connection problem<br>".mdb2_error($link2)."</b></font></center>";
		require('install.php');
		die();
	}
...
function ShowResults($req,$sortable=true,$modeCu=false,$modeRedon=false,$deletableP=true,$registrable=false,$teledeploy=false, $affect=false, $massProcessing=true, $allowCheckBoxes=false)
{
		global $l, $_GET;				
		global $LIKE;
		$deletable = check_param ($_GET, "multi")!=2 && $deletableP;
	if (check_param ($_GET, "multi")!=29 and check_param ($_GET, "multi")!=1)
		...
		{					
			...
			$req->where=str_replace("option$ind",$_SESSION["readServer"]->escape($_POST[$var]),$req->where); 
			// replace strings "optionX" in the request with their values from $_POST
			$ind++;
			$var="option".$ind;			
		}

		//TODO: MAJEUR, probleme si on trie une page alors qu'une autre page avec preferences est ouverte dans un onglet
		if(    isset($_SESSION["storedRequest"])   && (  (isset($_GET["c"])&&$_GET["c"]) || check_param ($_GET, "av") == 1 || check_param ($_GET, "suppCol") == 1 || isset($_GET["newCol"]))   ) 
			if(( check_param ($_GET, "multi")==29 && $_SESSION["storedRequest"]->label != $l->g(2) ) || ( check_param ($_GET, "lareq") == $l->g(583) && $_SESSION["storedRequest"]->label != $l->g(583) && $_GET["multi"]!=29 )) {
				unset( $_SESSION["storedRequest"] );
			}
			
		if(    isset($_SESSION["storedRequest"])   && (  (isset($_GET["c"])&&$_GET["c"]) || check_param ($_GET, "av") == 1 || check_param ($_GET, "suppCol") == 1 || isset($_GET["newCol"]))   ) 
		{
			if(check_param ($_SESSION, "c")==$_GET["c"]) { // If same column is sorted again
				if( check_param ($_GET, "rev") == 1 )
					$_GET["a"]= $_GET["a"] ? 0 : 1 ;
			}
			else {
				$_SESSION["c"]=$_GET["c"];
			...
			if ( check_param ($_GET, "c", "^[a-zA-Z0-9_\.\(\)]*$" )) {
				$ord = $_GET["c"] . $suffixe;
				$_SESSION["storedRequest"]->order = $ord;
				$toExec = getQuery( $_SESSION["storedRequest"], " $pcParPage OFFSET $beg") ;
			}
			else {
				$toExec = getQuery( $_SESSION["storedRequest"], " $pcParPage OFFSET $beg") ;
			}
		...
		else
		{
			...
			$beg = ($_SESSION["pageCur"]-1) * $pcParPage;
			if( ! $req->order and isset ($orderDefault)) $req->order = $orderDefault;
			
			$toExec = getQuery( $req, " $pcParPage OFFSET $beg") ;
			$_SESSION["storedRequest"]=$req;
		}
		
		$result = mdb2_query( $toExec, $_SESSION["readServer"] ) or die(mdb2_error($_SESSION["readServer"]));	
		...
		foreach ($_GET as $gk=>$gv) {
			if( in_array( $gk, array_merge($regularGets, $sortGets) ) ) 
				continue;
			$pref .= "&amp;" . urlencode ($gk) . "=" . urlencode($gv);
		}
		...
		foreach ($_GET as $gk=>$gv){
			...
			if( in_array( $gk, $regularGets ) ) 
				continue;
			$prefG .= "&amp;" . urlencode ($gk) . "=" . urlencode($gv);				
		...
		if (check_param ($_GET, "multi") == 24 and !isset($_GET['affect']))
		$prefG.="&affect=mach";
		$prefGssPage = $prefG;
		...
		if( !$modeCu && $count > 0) {
			...
			if( $columneditable) {
				...
				foreach( $_SESSION["availFieldList"] as $nomField=>$valField ) {
					if( ! array_case_search($valField,$_SESSION["currentFieldList"])	) {
						$countHl++;
		...
		if($modeRedon) {
			echo "<form id='idredon' name='redon' action='index.php?multi=6&c=".urlencode(check_param ($_GET, "c"))."&a=".urlencode(check_param ($_GET, "a"))."' method='POST'>
			...
			<script language=javascript>
				function confirme(did, typ, nam, target)
				{
					...
					if(confirm(mess+" "+(nam!=""?nam:did)+" ?"))
						if(typ == 1)
							window.location="index.php?<?php echo $pref?>&c=<?php echo (isset($_GET["c"])?urlencode($_GET["c"]):"1")?>&a=<?php echo (isset($_GET["a"])?urlencode($_GET["a"]):0); ?>&page=<?php echo urlencode(check_param ($_GET, "page"))?>&suppnet="+did;
						else
							window.location="index.php?<?php echo $pref?>&c=<?php echo (isset($_GET["c"])?urlencode($_GET["c"]):"1")?>&a=<?php echo (isset($_GET["a"])?urlencode($_GET["a"]):0); ?>&page=<?php echo urlencode(check_param ($_GET, "page"))?>&supp="+did;
				}
			</script>
		<?php 
			...
			{							
				...
				if($sortable) {	
					if( ! in_array($colname->name, array($l->g(28)) ) ) {// Not grouping requests
						$vraiNomChamp = array_case_search( $colname->name, $_SESSION["storedRequest"]->select );
						...
						if( ! $vraiNomChamp )
							$vraiNomChamp = array_case_search( $colname->name, $_SESSION["currentFieldList"] );							
						}
					else					
						$vraiNomChamp = strtolower(str_replace(' ','_',$l->g(28)));
					...
					if( isset($_SESSION["c"]) && ($vraiNomChamp == $_SESSION["c"] || "\'".$colname->name."\'" == check_param ($_SESSION, "c")) ) {
						if($a == 1 ) 
						...
						if( isset ($res[1]) && ($res[1] == $colname->name || $res[1] == $vraiNomChamp )) {
							if( $res[2] == "ASC" ) 
								echo "$hrefSort<img src='image/down.png'></a>";
							else 
                                echo "$hrefSort<img src='image/up.png'></a>";
					...
					if( sizeof($_SESSION["storedRequest"]->select)>3 && $columneditable && array_case_search( $colname->name , $_SESSION["currentFieldList"]))						
		...
		if( ($deletable||$modeRedon) && $req->countId == "h.id" and check_param ($_GET, "multi") != 29 )
		{
			echo "<td>&nbsp;</td>";
		}
		if( $teledeploy ) {
		...
		$resIdGroup = mdb2_query( $sqlIdGroup, $_SESSION["readServer"] );
		$listIdGroup="";
			while( $valIdGroup = mdb2_fetch_assoc( $resIdGroup, CASE_UPPER ) ) {
				$listIdGroup .= $valIdGroup['ID'].",";
			}
			$listIdGroup=substr($listIdGroup,0,-1);
		while($item = mdb2_fetch_assoc($result))
		{	
			flush();
			...
			foreach($tabChamps as $chmp) {
                echo "<td align='center'>";								
                $isLink = FALSE;
				if(strcasecmp($chmp, TAG_LBL) == 0)
				{
					$leCuPrec=$item[$chmp];					
				}
				else if(strcasecmp($chmp, $l->g(23)) == 0 and isset($item["h.id"]))
				{					
				...
				else if(strcasecmp ($chmp, $l->g(577)) == 0 and isset($item["h.id"]))
				{
				...
				else if(strcasecmp ($chmp, str_replace(' ','_',$l->g(28))) == 0)
				{
					echo "<a href='?cuaff=$leCuPrec'>";
					$isLink=TRUE;
				}
				else if(strcasecmp ($chmp, $l->g(49)) == 0 and $_GET['multi'] == 21)
				{
                                        $sql="select id from download_enable where fileid=?";
                                        $resGetId = mdb2_query( $sql, $_SESSION["readServer"], "text", urlencode ($item["timestamp"]));
    	                                if( $valGetId = mdb2_fetch_assoc( $resGetId ) ) {
				...
				echo htmlspecialchars($item[$chmp]);
			...
			if( $deletable && isset($item["h.id"]) and check_param ($_GET, "multi") != 29) {
				echo "<td align=center><a href='#' OnClick='confirme(\"".$item["h.id"]."\",0,".(isset($item[strtolower($l->g(23))])?"\"".htmlentities($item[strtolower ($l->g(23))])."\"":"\"\"").",".(check_param($_GET, "lareq")==$l->g(583)?2:1).");'><img src=image/supp.png></a></td>";
			}
			else if( $deletable && isset($item[strtolower ($l->g(95))]) ) {
				echo "<td align=center><a href='#' OnClick='confirme(\"".$item[strtolower ($l->g(95))]."\",1,\"\",1);'><img src=image/supp.png></a></td>";
			}
...
//affichage des statistiques pour l'activation de paquet
			if( $teledeploy ) {
				...
				$sqlStat="SELECT ID 
							...
							WHERE fileid=?";
				$resStat = mdb2_query( $sqlStat, $_SESSION["readServer"], "text", $item["timestamp"]);
				$listIdstat="";
				while( $valStat = mdb2_fetch_assoc( $resStat, CASE_UPPER ) ) {
					$listIdstat .= $valStat["ID"].",";
				}
				$listIdstat=substr($listIdstat,0,-1);
				if ($listIdstat != ""){
					$sqlStatnb="select count(*) as nb, " . $_SESSION["readServer"]->function->substring ("tvalue", 1, 4) . " as design
								 from devices 
					...
					$sqlStatnb.= "	and name='DOWNLOAD' 
								...
								group by design";
					$resStatnb = mdb2_query($sqlStatnb, $_SESSION["readServer"]);
					if ($resStatnb != ""){
						while ($valStatnb = mdb2_fetch_assoc( $resStatnb)){
							$statResult[$valStatnb['design']] = $valStatnb['nb'];
						}
					}
				}
				if (!is_array ($statResult) or check_param ($statResult, "") == ""){
				$statResult[""] = 0;		
				}
				
				if (check_param ($statResult, "SUCC") == ""){
				$statResult["SUCC"] = 0;		
				}
					
				if (check_param ($statResult, "ERR_") == ""){
				$statResult["ERR_"] = 0;		
				}
...
//				$valTot = mysql_fetch_array( $resTot );
				...
				echo "<td align='center'><a href=\"tele_compress.php?timestamp=".array_shift(array_values($item))."\" target=_blank><img src='image/archives.png'></a></td>";
				echo "<td align='center'>";
				if( $statTotal > 0 )
					echo "<a href=\"tele_stats.php?stat=".array_shift(array_values($item))."\" target=_blank><img src='image/stat.png'></a>";
				else
					echo "<b>-</b>";
					
				echo "</td><td align='center'><a href='index.php?multi=21&actpack=".array_shift(array_values($item))."'><img src='image/activer.png' ></a></td><td align='center'><a href=# OnClick='javascript:ruSure(\"index.php?multi=21&suppack=".array_shift(array_values($item))."\");return false;'><img src=image/supp.png></a></td>";	
			}
			//Package affectation query
			else if( $affect ) {
			
				if( $affect == 2 ) {
					echo "<td align='center'><a href=# OnClick='javascript:ruSure(\"index.php?multi=26&popup=1&timestamp=".$_GET['timestamp']."&suppack=".array_shift(array_values($item))."\");return false;'><img src=image/supp.png></a></td>";
					echo "<td align='center'><a href='index.php?multi=30&id=".array_shift(array_values($item))."' target='_top'><img src=image/Gest_admin1.png></a></td>";
				...
				if( $affect != 2) {
					...
					if( isset( $tbd ) ) {
						...
						foreach( $tbd as $compAffect ) {
							echo "compAffect".$compAffectIndex."=" . urlencode ($compAffect). "&";
							$compAffectIndex++;
						}
					}
					else
						echo ($affect==3 ? "systemid=".$_GET["systemid"]."&":"");
					echo "multi=24&affpack=".array_shift(array_values($item))."\");return false;'><img src='image/Gest_admin1.png'></a></td>";	
				}
			}
			...
			if( check_param ($_GET, "multi")==29 ) {
				$reqStat = "SELECT static FROM groups_cache WHERE hardware_id=? AND group_id=?";

				$resStat = @mdb2_query($reqStat, $_SESSION["readServer"], array ("integer", "integer"), array ($item["h.id"], $_GET["systemid"]));
				$valStat = @mdb2_fetch_assoc($resStat);
		...
		if( $uneMachine ) {
			...
			if($massProcessing && $req->label != $l->g(583) && $req->label != $l->g(2) ) {
				...
				if( check_param ($_GET, "multi") == 1 ) {
					...
					$resGroups = mdb2_query( $reqGroups, $_SESSION["readServer"] );
					//list of choise
					...
					if ( $_SESSION["lvluser"]==SADMIN ){
						...
						$resGroupsServers = mdb2_query( $reqGroupsServers, $_SESSION["readServer"] );
						$groupListServers = "";
						while( $valGroupsServers = mdb2_fetch_assoc( $resGroupsServers ) ) {
		...
		printNavigation( $prefGssPage, $numPages);
		if (check_param ($_GET, "systemid") !== NULL) {
		$reqGrpStat = "SELECT request FROM groups WHERE hardware_id=?";
		$resGrpStat = @mdb2_query($reqGrpStat, $_SESSION["readServer"], "integer", $_GET["systemid"]);
		$valGrpStat = @mdb2_fetch_assoc($resGrpStat);
		if( check_param ($_GET, "multi") == 29 && $allowCheckBoxes ) {
...
function getCount( $req ) {
	...
	if( stripos(" ".$req->where , ' ' . $_SESSION["readServer"]->quoteIdentifier('a') . '.')===FALSE&&
		stripos(" ".$req->where , ' ' . $_SESSION["readServer"]->quoteIdentifier('b') . '.')===FALSE ) {
...
function getQuery( $req, $limit ) {
	$ech = $_SESSION["debug"];
	$resPrelim = mdb2_query( getPrelim( $req, $limit ) , $_SESSION["readServer"]);
	...
	$lesIn = '';
	while( $valPrelim = mdb2_fetch_assoc($resPrelim) ) {
		if( !$prem) $lesIn .= ",";

		$lesIn .= $_SESSION["readServer"]->quote ($valPrelim[strtolower ($req->linkId)], "text");
		$prem = false;
	}
...
function dateFromMysql($v) {
	...
	if( $l->g(269) == "%m/%d/%Y" )
		...
		$ret = sprintf("%04d-%02d-%02d", $v, $v[5].$v[6], $v[8].$v[9]);
	return $ret;
}
...
function dateTimeFromMysql($v) {
	...
	if( $l->g(269) == "%m/%d/%Y" )
		...
		$ret = sprintf("%04d/%02d/%02d %02d:%02d:%02d", $v, $v[5].$v[6], $v[8].$v[9], $v[11].$v[12],$v[14].$v[15],$v[17].$v[18]);
	return $ret;
}

function dateToMysql($date_cible) {
	return $date_cible; // We use ISO date formatting exclusivly, making this function a no-op.
}
...
function deleteNet($id) {
	mdb2_query("DELETE FROM network_devices WHERE macaddr=?", $_SESSION["writeServer"], "text", strtoupper ($id));
}
...
function deleteDid($id, $checkLock = true, $traceDel = true, $silent=false) {
	...
	if( ! $checkLock || lock($id) ) {	
		$resId = mdb2_query("SELECT deviceid,name FROM hardware WHERE id=?",$_SESSION["readServer"], "integer", $id) or die(mdb2_error());
		$valId = mdb2_fetch_assoc($resId);
		$idHard = $id;
		$did = $valId["deviceid"];
		if( $did ) {	
			//Deleting a network device
			if( strpos ( $did, "NETWORK_DEVICE-" ) === false ) {
				$resNetm = @mdb2_query("SELECT macaddr FROM networks WHERE hardware_id=?", $_SESSION["writeServer"], "integer", $idHard) or die(mdb2_error());
				while( $valNetm = mdb2_fetch_assoc($resNetm)) {
					@mdb2_query("DELETE FROM netmap WHERE mac=?", $_SESSION["writeServer"], "text", strtoupper ($valNetm["macaddr"])) or die(mdb2_error());
				}		
			}
			...
			elseif($did == "_SYSTEMGROUP_" or $did == '_DOWNLOADGROUP_'){//Deleting a group
				...
				mdb2_query($del_groups_server_cache, $_SESSION["writeServer"], 'integer', $idHard) or die(mdb2_error());
				mdb2_query("DELETE FROM groups WHERE hardware_id=?", $_SESSION["writeServer"], "integer", $idHard) or die(mdb2_error());
				$resDelete = mdb2_query("DELETE FROM groups_cache WHERE group_id=?", $_SESSION["writeServer"], "integer", $idHard) or die(mdb2_error());
				$affectedComputers = mdb2_affected_rows( $_SESSION["writeServer"] );
			...
			foreach ($tables as $table) {
				mdb2_query("DELETE FROM $table WHERE hardware_id=?;", $_SESSION["writeServer"], "integer", $idHard) or die(mdb2_error());		
			}
			mdb2_query("delete from download_enable where SERVER_ID=?", $_SESSION["writeServer"], "integer", $idHard) or die(mdb2_error($_SESSION["writeServer"]));
			
			mdb2_query("DELETE FROM hardware WHERE id=?", $_SESSION["writeServer"], "integer", $idHard) or die(mdb2_error());
			//Deleted computers tracking
			if($traceDel && mdb2_num_rows(mdb2_query("SELECT IVALUE FROM config WHERE IVALUE>0 AND NAME='TRACE_DELETED'", $_SESSION["writeServer"]))){
				mdb2_query("insert into deleted_equiv(DATE,DELETED,EQUIVALENT) values(CURRENT_TIMESTAMP,?,NULL)", $_SESSION["writeServer"], "text", $did) or die(mdb2_error());
			}
		}
		//Using lock ? Unlock
		if( $checkLock ) 
...
function lock($id) {
	...
	$reqClean = "DELETE FROM locks WHERE " . $_SESSION["writeServer"]->function->unixtimestamp("since") . " < (" . $_SESSION["writeServer"]->function->unixtimestamp("CURRENT_TIMESTAMP") . " - 3600)";
	$resClean = mdb2_query($reqClean, $_SESSION["writeServer"]) or die(mdb2_error());
	
	$reqLock = "INSERT INTO locks(hardware_id, since) VALUES (?, CURRENT_TIMESTAMP)";
	if( $resLock = mdb2_query($reqLock, $_SESSION["writeServer"], "integer", $id) or die(mdb2_error()))
		return( mdb2_affected_rows ( $_SESSION["writeServer"] ) == 1 );
	else return false;
}
...
function unlock($id) {
	...
	$reqLock = "DELETE FROM locks WHERE hardware_id=?";
	$resLock = mdb2_query($reqLock, $_SESSION["writeServer"], "integer", $id) or die(mdb2_error());
	return( mdb2_affected_rows ( $_SESSION["writeServer"] ) == 1 );
}
...
function getConstructor( $mac ) {	
	...
	return ( ucwords(strtolower( check_param ($_SESSION["mac"], $beg))) );
}
...
function getIds($reqSid) {
	...
	return isset ($idNotIn) ? $idNotIn : NULL;
}
?>
