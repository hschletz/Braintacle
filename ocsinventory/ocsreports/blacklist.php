	...
	<script language=javascript>
		function confirme(did){
			if(confirm("<?php echo $l->g(640)?> id "+did+" ?"))
				window.location="index.php?multi=<?php echo $_GET["multi"]?>&choise=<?php echo check_param ($_POST, 'choose_blacklist')?>&suppAcc="+did;
		}
...
if (check_param ($_POST, 'choose_blacklist') == 'mac'){
...
}elseif (check_param ($_POST, 'choose_blacklist') == 'serial'){
...
if(check_param ($_GET, "suppAcc")){
	mdb2_query ("DELETE FROM $table WHERE id=?", $_SESSION["writeServer"], "integer", $_GET["suppAcc"]);
}
//cas of add mac address or serial
if(check_param ($_POST, "enre")){	
	...
	if ($_POST['choose_blacklist'] == 'mac')
		$mac_serial = strtoupper ($mac_serial);
	
	//if no error
	if (!isset($erreur)){
		//insert into table the new mac address
		$sql = "INSERT INTO $table ($champ) VALUES (?)";
		//no error
		if (mdb2_query($sql, $_SESSION["writeServer"], "text", $mac_serial))
		//message OK
...
if (isset($print_entete)){
	...
	while ($i<=$nbr_champs)	
	{
		echo "<input size=".$maxlength." maxlength=".$maxlength." name='champ_".$i."' ".$maj." value='".check_param ($_POST, 'champ_'.$i)."' ".(isset($erreur['champ_'.$i])? "style='color:white; background-color:red;'" : "").">";
		if ($i != $nbr_champs)
		echo ":";
	$i++;
	}
	...
	$result = mdb2_query($sql_list_black, $_SESSION["readServer"]) or die(mdb2_error($_SESSION["readServer"]));
	$title=$l->g(233);
	$i=0;
	foreach ($result->getColumnNames (true) as $colname)
		$entete[$i++]=strtoupper ($colname);
		$entete[$i]="Sup";
	$i=0;
