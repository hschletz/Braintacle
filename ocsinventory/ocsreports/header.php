...
$_get_multi = check_param ($_GET, "multi", "(^[0-9]*|console)$");
$_get_mode = check_param ($_GET, "mode", "^[0-9]*$");

// update checking
$_SESSION["readServer"]->expectError();
$resUpd = $_SESSION["readServer"]->query ("SELECT tvalue FROM config WHERE name='GUI_VERSION'");
$_SESSION["readServer"]->popExpect();
if (PEAR::isError ($resUpd))
$valUpd = null;
else
$valUpd = @mdb2_fetch_assoc($resUpd);
...
<?php if($_get_multi == 3 && $_get_mode == 1) {?>
...
if( !isset($_GET["popup"] )) {
?>
<table class='headfoot' border='0' <?php if (isset ($ban_head)) echo "style='display:none;'"?>>
<tr height=25px>
...
if($_SESSION["debug"]==1)
	...
	if(isset($_POST["login"])) {				
		$req="SELECT id, accesslvl, passwd FROM operators WHERE id=?";
		$res=mdb2_query($req,$_SESSION["readServer"], "text", $_POST["login"]) or die(mdb2_error());
		
		if($row=@mdb2_fetch_object($res))
		{
                     // DL 25/08/2005
	...
	if ( !isset($_SESSION["loggeduser"]) && $dir = @opendir("languages")) {
		...
		while($filename = readdir($dir)) {
			...
			echo "<a title='$langue' href=\"index.php?av=1&multi=$_get_multi&c=" . check_param ($_GET, "c") . "&a=" . check_param ($_GET, "a") . "&lang=$langue\"><img src =\"languages/$langue.png\" width=\"20\" height=\"15\"></a>&nbsp;";
		}
		closedir($dir);
	}
	...
	if( in_array($_get_multi,$limitedAccess) && $_SESSION["lvluser"]!=1) {
