...
if (isset($_POST['RULE_AFFECT'])){
	...
	$res_toute_machines = mdb2_query( $lareq, $_SESSION["readServer"] ) or die(mdb2_error($_SESSION["readServer"]));

	while( $val_toute_machines = mdb2_fetch_assoc($res_toute_machines)) {
			$ID_HARDWARE[$val_toute_machines['h.id']]=$val_toute_machines['h.id'];
	}
	
	$sql="select PRIORITY,CFIELD,OP,COMPTO,SERV_VALUE from download_affect_rules where rule=? order by PRIORITY";
	$res_rules = mdb2_query( $sql, $_SESSION["readServer"], "integer", $_POST['RULE_AFFECT']) or die(mdb2_error($_SESSION["readServer"]));
	
	while( $val_rules = mdb2_fetch_assoc($res_rules, CASE_UPPER)) {
...
if( ! isset($_SESSION["saveRequest"])) {
	$_SESSION["saveRequest"] = check_param ($_SESSION, "storedRequest");
}
...
if( check_param ($_GET, "retour") == 1 || (isset($_GET["affpack"]) && $ok) ) {
...
{
	$visu='none';
	if (check_param ($_GET, 'affect')=="mach"){
	$selected="selected";
	$visu='block';
	}
...
$sql="select hardware.NAME as GROUP_SERVER,
			...
			hardware.NAME
union
select " . $_SESSION["readServer"]->quote ("<font color=red><b>" . $l->g(660) . "</b></font>", "text") . " as GROUP_SERVER,
			count(d.FILEID) as NOMBRE,
			d.FILEID as TIMESTAMP ,
			d.NAME as NOM,
			PRIORITY as PRIORITE,
...
function setPack( $packid ) {
		...
		if( isset($_GET["systemid"])) {
			...
			if( ! @mdb2_query( "INSERT INTO devices(HARDWARE_ID, NAME, IVALUE) VALUES(?, 'DOWNLOAD', ?)", $_SESSION["writeServer"], array ("integer", "integer"), array ($val["h.id"], $packid) )) {
			...
			foreach( $_GET as $key=>$val ) {
				if( strpos ( $key, "compAffect" ) !== false ) {
					if( ! @mdb2_query( "INSERT INTO devices(HARDWARE_ID, NAME, IVALUE) VALUES(?, 'DOWNLOAD', ?)", $_SESSION["writeServer"], array ("integer", "integer"), array ($val, $packid) )) {
						echo "<br><center><font color=red><b>ERROR: Database connection problem<br>".mdb2_error($_SESSION["writeServer"])."</b></font></center>";
						return false;
					}
				}
	...
	function resetPack( $packid ) {
		...
		if( isset($_GET["systemid"])) {
			$val["h.id"] = $_GET["systemid"];
			if( ! @mdb2_query( "DELETE FROM devices WHERE name='DOWNLOAD' AND IVALUE=? AND hardware_id=?", $_SESSION["writeServer"], array ("integer", "integer"), array ($packid, $val["h.id"]))) {
				echo "<br><center><font color=red><b>ERROR: Database connection problem<br>".mdb2_error($_SESSION["writeServer"])."</b></font></center>";
				return false;
			}
		}
			...
			foreach( $_GET as $key=>$val ) {
				if( strpos ( $key, "compAffect" ) !== false ) {
					if( ! @mdb2_query( "DELETE FROM devices WHERE name='DOWNLOAD' AND IVALUE=? AND hardware_id=?", $_SESSION["writeServer"], array ("integer", "integer") , array ($packid, $val))) {
						echo "<br><center><font color=red><b>ERROR: Database connection problem<br>".mdb2_error($_SESSION["writeServer"])."</b></font></center>";
						return false;
					}
				}
