...
<?php 
	...
	if( isset($_POST["nom"]) ) {
		...
		$verifN = "SELECT fileid FROM download_available WHERE name=?";
		$resN = mdb2_query( $verifN, $_SESSION["readServer"], "text", $_POST["nom"]) or die(mdb2_error());
		
		if( mdb2_num_rows( $resN ) == 0 ) {
		...
		//fragmenter
		$fname = $document_root."/download/" . check_param ($_POST, "id", "^[0-9]+$") . "/tmp";
		...
		mdb2_query( "DELETE FROM download_available WHERE FILEID='".$_POST["id"]."'", $_SESSION["writeServer"]);
		$req = "INSERT INTO download_available(FILEID, NAME, PRIORITY, FRAGMENTS, SIZE, OSNAME, COMMENT) VALUES
		( ?, ?, ?, ?, ?, ?, '' )";
		
		mdb2_query( $req, $_SESSION["writeServer"], array ("text", "text", "integer", "integer", "integer", "text"), array ($_POST["id"], $_SESSION["down_nom"], $_SESSION["down_priority"], $_POST["nbfrags"], $size, $_SESSION["down_os"]));
		echo mdb2_error();
