...
if($_SESSION["lvluser"]==SADMIN||$_SESSION["lvluser"]==LADMIN) {
	...
	$mescQuery = "SELECT DISTINCT(tag) FROM tags WHERE login=?";
	$mescRes = mdb2_query ($mescQuery, $_SESSION["readServer"], "text", $user);
	while( $mescVal = mdb2_fetch_assoc ($mescRes) ) {
		$mach[] = $_SESSION["readServer"]->quote ($mescVal["tag"], "text");
	}
	if (!isset($mach)){
...
/**********************************************/
	...
	$sql = "deviceid<>'_SYSTEMGROUP_' AND " . mdb2_quote_identifier("a.".TAG_NAME) . " = 'option0' ";
	if( $mesMachines != "" ) $sql .= " AND ";
	$sql .= $mesMachines; 
	$whereId = "h.id";
	$linkId = "h.id";
