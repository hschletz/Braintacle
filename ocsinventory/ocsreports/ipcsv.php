...
//echo $lareq;die();
$result=mdb2_query($lareq, $_SESSION["readServer"]) or die(mdb2_error($_SESSION["readServer"]));
...
$cols = $result->getColumnNames(true);
foreach ($cols as $col) {
	$nameIndex = ($col == strtolower($l->g(24)) ? $colnb : $nameIndex);
	$colnb++;	
}
...
while( $cont = mdb2_fetch_assoc($result) ) {
	writeTab($cont,$nameIndex);
}

header("Content-Length: ".strlen($toBeWritten));
echo $toBeWritten;

function writeTab ($tab,$rgn=-1) {	
	global $toBeWritten;
	$colnb = 0;
	foreach($tab as $t) {		
