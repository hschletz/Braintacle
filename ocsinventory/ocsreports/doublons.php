...
$filtreSSN = " AND b.ssn NOT IN (SELECT serial FROM blacklist_serials) ";
$filtreMAC = " AND n1.macaddr NOT IN (SELECT macaddress FROM blacklist_macaddresses) ";
...
$where[7] = "$whereBase AND b2.hardware_id <> b.hardware_id AND b2.hardware_id = h2.id AND b2.assettag = b.assettag AND b.assettag <> '' AND b.assettag IS NOT NULL ";
$group[7] = "h.id";
$order[7] = "b.assettag";
...
if(isset($_POST["subredon"])) {
	for( $i = 1 ; $i <= $_POST["maxredon"] ; $i++) {
		...
		$res = mdb2_query ("SELECT deviceid,id,lastcome FROM hardware WHERE id=?", $_SESSION["readServer"], "integer", $_POST["ch$i"]) or die(mdb2_error($_SESSION["readServer"]));
...
if(check_param ($_SESSION, "typ")&&!isset($_POST["typ"])) {
...
if($_POST["typ"]!=$l->g(192)) {
	...
	for($j=1;$j<=6;$j++) {
		...
		$rres = mdb2_query( $rrqs, $_SESSION["readServer"]);
	
		$valr = mdb2_fetch_row($rres);
...
function fusionne($afus) {
	...
	if($afus[$minInd]["deviceid"]!="") {
		...
		if( $okLock ) {
			//TRACE_DELETED
			if(mdb2_num_rows(mdb2_query("SELECT * FROM config WHERE ivalue>0 AND name='TRACE_DELETED'", $_SESSION["readServer"]))){
				foreach($afus as $a) {	
					if($afus[$maxInd]["deviceid"]==$a["deviceid"]){continue;}
					mdb2_query ("INSERT INTO deleted_equiv (date,deleted,equivalent) VALUES (CURRENT_TIMESTAMP,?,?)", $_SESSION["writeServer"], array ("text", "text"), array ($a["deviceid"], $afus[$maxInd]["deviceid"]) );
				}
			}
			
			//KEEP OLD QUALITY,FIDELITY AND CHECKSUM
			$persistent_req = mdb2_query ("SELECT checksum,quality,fidelity FROM hardware WHERE id=?", $_SESSION["readServer"], "integer", $afus[$minInd]["id"] );
					
			$reqDelAccount = "DELETE FROM accountinfo WHERE hardware_id=?";
			mdb2_query ($reqDelAccount, $_SESSION["writeServer"], "integer", $afus[$maxInd]["id"]) ;
			...
			foreach( $keep as $tableToBeKept ) {
				$reqRecupAccount = "UPDATE ".$tableToBeKept." SET hardware_id=? WHERE hardware_id=?";
				//echo $reqRecupAccount;
				mdb2_query ($reqRecupAccount, $_SESSION["writeServer"], array ("integer", "integer"), array ($afus[$maxInd]["id"], $afus[$minInd]["id"]));
			}						
			...
			$persistent_values = mdb2_fetch_row($persistent_req);
			mdb2_query ("UPDATE hardware SET quality=?, fidelity=?, checksum=checksum|? WHERE id=?", $_SESSION["writeServer"], array ("decimal", "integer", "integer", "integer"), array ($persistent_values[1], $persistent_values[2], $persistent_values[0], $afus[$maxInd]["id"]));
