...
function tri($sql)
{
	...
	if ($_GET['sens']){
	$sens=check_param ($_GET, "sens", "^(ASC|DESC)$", null, false);
	$col=check_param ($_GET, "col", "^[a-zA-Z0-9]+$");
	}
	else{
	$sens="ASC";
	$col=1;
...
//function to dump data as an HTML table
/*
...
$num_lig=0;
/* function related to show_modif
 * which allows to create a line in the table of modification/addition
 * $title = titre a l'affichage du champ
 * $value_default = - for a text entry field, the default value for the field
 * 					- for a list field, list of values
 * $input_name = field to retrieve from $_POST
 * $input_type = 0 : <input type='text'>
 * 				 1 : <textarea>
 * 				 2 : <select><option>
 * $donnees = table containing all the data
 * $nom_form = if selecting an entry in the list should trigger a reload, the name of the form to reload
*/
...
/*
 * function related to tab_modif_values which allows to post the field defined with the function champsform
 * $name = nom du champ
 * $input_name = field to retrieve from $_POST
 * $input_type = 0 : <input type='text'>
 * 				 1 : <textarea>
 * 				 2 : <select><option>
 * $input_reload = if selecting an entry in the list should trigger a reload, the name of the form to reload
 * 
 */
function show_modif($name,$input_name,$input_type,$input_reload = "")
{
	...
	$input_reload = htmlspecialchars ($input_reload);
	...
	elseif($input_type ==2){
		...
		foreach ($name as $key=>$value){
			$champs.= "<option value=\"" . htmlspecialchars ($key) . "\"";
			if (check_param ($_POST, $input_name) == $key )
			$champs.= " selected";
			$champs.= ">" . htmlspecialchars ($value) . "</option>";
		}
		$champs.="</select>";
		return $champs;
	}
...
function tab_modif_values($tab_name,$tab_typ_champ,$tab_hidden,$title="",$comment="")
{
	...
	{
		echo "<tr><td>".$values."</td><td>".check_param ($tab_typ_champ[$key], 'COMMENT_BEFORE').show_modif($tab_typ_champ[$key]['DEFAULT_VALUE'],$tab_typ_champ[$key]['INPUT_NAME'],$tab_typ_champ[$key]['INPUT_TYPE'],check_param ($tab_typ_champ[$key], 'RELOAD')).check_param ($tab_typ_champ[$key], 'COMMENT_BEHING')."</td></tr>";
	}
...
function nb_page($form_name,$taille_cadre='80',$bgcolor='#C7D9F5',$bordercolor='#9894B5'){
	global $_POST,$l;
	if (check_param ($_POST, "old_pcparpage") != check_param ($_POST, "pcparpage"))
	$_POST['page']=0;
	...
	echo $pcParPageHtml;
	if (isset($_POST["pcparpage"])){
		$limit=check_param ($_POST, "pcparpage", "^[0-9]+$");
		$deb_limit=check_param ($_POST, "page")*$_POST["pcparpage"];
	$fin_limit=$limit;		
	}
...
function show_page($valCount,$form_name){
	...
	if ($nbpage >1){
	$up=check_param ($_POST, 'page')+1;
	$down=check_param ($_POST, 'page')-1;
	echo "<table align='center' width='99%' border='0' bgcolor=#f2f2f2>";
	echo "<tr><td align=center>";
	if (check_param ($_POST, 'page') > 0)
	...
	if (check_param ($_POST, 'page')< $nbpage-1)
	echo "<img src='image/proch24.png' OnClick='pag(\"".$up."\",\"page\",\"".$form_name."\")'>";
	
	}
	echo "</td></tr></table>";
	echo "<input type='hidden' id='page' name='page' value='" . check_param ($_POST, "page") . "'>";
...
{
	global $_POST;
	if (check_param ($_POST, "old_$post_name") != check_param ($_POST, $post_name)){
	$_POST['page']=0;
	}
	...
	$i=0;
	  foreach($def_onglets as $key=>$value){
	  	...
	  	echo "<li ";
	  	if (is_numeric(check_param ($_POST, $post_name))){
			...
			if (check_param ($_POST, $post_name) === $key or (!isset($_POST[$post_name]) and $current != 1)){
				 echo "id='current'";  
	 			 $current=1;
			}
		}
	...
	echo "<input type='hidden' id='".$post_name."' name='".$post_name."' value='" . check_param ($_POST, $post_name) . "'>";
	echo "<input type='hidden' id='old_".$post_name."' name='old_".$post_name."' value='" . check_param ($_POST, $post_name) . "'>";
	}
}
...
function gestion_col($entete,$data,$list_col_cant_del,$form_name,$tab_name,$list_fields,$default_fields,$id_form='form'){
	global $_POST,$l;
	if (check_param ($_POST, 'SUP_COL') != "" and isset($_POST['SUP_COL']))
	unset($_SESSION['col_tab'][$tab_name][$_POST['SUP_COL']]);
	if (check_param ($_POST, 'restCol'.$tab_name)){
		$_SESSION['col_tab'][$tab_name][$_POST['restCol'.$tab_name]]=$_POST['restCol'.$tab_name];
	}
	if (check_param ($_POST, 'RAZ') == "RAZ")
	...
	$select_restCol .= "<option".(check_param ($_POST, "restCol".$tab_name) == "DEFAULT" ? " selected" : "")." >".$l->g(32)."...</option>";
	$countHl=0;
	if ($list_rest != ""){
		 ksort($list_rest,SORT_STRING);
		foreach( $list_rest as $lbl_field => $name_field) {
			$select_restCol .=  "<option".(check_param ($_POST, "restCol".$tab_name) == $lbl_field ? " selected" : "").($countHl%2==0?" class='hi'":"")." value=".$lbl_field.">$name_field</option>";
			$countHl++;
		}
	}
...
function tab_req($table_name,$list_fields,$default_fields,$list_col_cant_del,$queryDetails,$form_name,$width='100')
{
	...
	if (check_param ($_POST, 'tri2') == "" or !in_array($_POST['tri2'],$list_fields))
	$_POST['tri2']=1;
	if (check_param ($_POST, 'sens') == "")
	$_POST['sens']='ASC';
	$queryDetails.= " order by " . check_param ($_POST, "tri2", "^[a-zA-Z0-9]+$") ." " . check_param ($_POST, "sens", "^(ASC|DESC)$", null, false);
	$resultDetails = mdb2_query($queryDetails, $link) or mdb2_error($link);
	$num_rows_result = mdb2_num_rows($resultDetails);
	$_SESSION['cvs'][$table_name]=$queryDetails;	
	$queryDetails.=" limit ".$limit["END"]." OFFSET ".$limit["BEGIN"];
	$resultDetails = mdb2_query($queryDetails, $link) or mdb2_error($link);
	$i=0;	
	
	while($item = mdb2_fetch_object($resultDetails, CASE_UPPER)){
		// Escape data to prevent possible XSS
