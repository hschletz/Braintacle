	...
	echo "<br><font size=1 color=green><i>".$lbl."</i></font></td><td align='left' width='150px'>";
 	if ($type=='radio' or $type=='checkbox'){
 		if ($data_hidden != ''){
			...
			$value_hidden = $data_hidden['HIDDEN'];
 		}
		else $value_hidden = NULL;
 		//echo "<br>hidden ==".$data_hidden['HIDDEN']."      value ==".$data['VALUE'];
 		if ($value_hidden==check_param ($data, 'VALUE'))
 		$display="block";
 		else
 		$display="none";
 		...
 		foreach ($data as $key=>$value){
 			if ($key !== 'VALUE' and $key !== 'CHECK'){
	 			...
	 			if (check_param ($data, 'VALUE') == $key or isset($data['CHECK'][$key]))
	 			echo "checked";
	 			echo ">".$value; 
	 			if ($data_hidden != '' and  $data_hidden['HIDDEN'] == $key){
	 				echo "<div id='".$name."_div' style='display:".$display."'><input type='text' size='".(check_param ($data_hidden, "SIZE")?$data_hidden['SIZE']:"3")."' maxlength='".(check_param ($data_hidden, "SIZE")?$data_hidden['SIZE']:"2")."' id='".$name."_edit' name='".$name."_edit' value='".$data_hidden['HIDDEN_VALUE']."' ".check_param ($data_hidden, 'JAVASCRIPT').">" . check_param ($data_hidden, "END") . "</div>"; 	
	 			}
	 			echo "<br>";
 			}
 		}
		...
		$ajout_readonly = "";
 		echo check_param ($data, "BEGIN") . "<input ".$ajout_readonly."  type='text' name='".$name."' id='".$name."' value='".check_param ($data, 'VALUE')."' size=".$data['SIZE']." maxlength=".$data['MAXLENGHT']." ".check_param ($data, 'JAVASCRIPT').">".check_param ($data, 'END'); 		
 	}elseif($type=='text'){
 		echo $data[0];
 	}elseif($type=='select'){
...
function verif_champ(){
	...
	$i=0;
	$tab_error = NULL;
	while (check_param ($supp1, $i)){
		if (isset($_POST[$supp1[$i]]) and $_POST[$supp1[$i]] < 1)
			$tab_error[$supp1[$i]]='1';
		$i++;
	}
	$i=0;
	while (check_param ($supp10, $i)){
		if (isset($_POST[$supp10[$i]]) and $_POST[$supp10[$i]] < 10)
...
function look_default_values($field_name){
	...
	$resdefaultvalues = mdb2_query( $sql, $_SESSION["readServer"]);
	$result = NULL;
...
function look_perso_values($champs,$origine,$systemid){
	...
	$sql_value_idhardware="select * from devices where name != 'DOWNLOAD' and hardware_id=?";
	$result_value = mdb2_query($sql_value_idhardware, $_SESSION["readServer"], "integer", $systemid) or die(mdb2_error($_SESSION["readServer"]));
...
function update_default_value($POST){
	...
	foreach ($POST as $key=>$value){
		unset($sql);
		unset($optexist);
		$sql_exist=" select NAME from config where NAME=?";
		$result_exist = mdb2_query($sql_exist, $_SESSION["readServer"], "text", $key) or die(mdb2_error($_SESSION["readServer"]));
		while($value_exist=mdb2_fetch_assoc($result_exist, CASE_UPPER)) {
		...
		if ($value != ""){
				if ($key != 'onglet'
					...
					and $key != 'IPDISCOVER_IPD_DIR' and $key != 'IPDISCOVER_IPD_DIR_edit'){
						if (isset($optexist))
						$sql="update config set IVALUE = ? where NAME =?";
						else
						$sql="insert into config (IVALUE, NAME) values (?,?)";
				...
				elseif(($key == 'DOWNLOAD_PACK_DIR' and $value == 'CUSTOM')
					...
					$sql="update config set TVALUE = ? where NAME =?";
					elseif(trim($POST[$key."_edit"]) != '')
					$sql="insert into config (TVALUE, NAME) values (?,?)";
					$types = array("text", "text");
					$values = array($POST[$key."_edit"], $key);
				}
				...
				elseif($key == 'FREQUENCY' and $value != 'CUSTOM'){
					if ($value == 'ALWAYS') $ivalue=0; else $ivalue=-1;
						if (isset($optexist))
						$sql="update config set IVALUE = ".$ivalue." where NAME =?";
						else
						$sql="insert into config (IVALUE, NAME) values (".$ivalue.",?)";
						$types = "text";
						$values = $key;
				}
				...
				elseif($key == 'IPDISCOVER' and $value == 'OFF'){
					if (isset($optexist))
					$sql="update config set IVALUE = 0 where NAME =?";
					else
					$sql="insert into config (IVALUE, NAME) values (0,?)";
					$types = "text";
					$values = $key;
				}
		...
		if ($POST['onglet'] == $l->g(499) and $i==0){
			$sql="update config set IVALUE = ".$i." where NAME ='AUTO_DUPLICATE_LVL'";
			if( ! @mdb2_query( $sql, $_SESSION["writeServer"] )) {
						echo "<br><center><font color=red><b>ERROR: Database connection problem<br>".mdb2_error($_SESSION["writeServer"])."</b></font></center>";
						return false;
					}		
		}
...
function auto_duplicate_lvl_poids($value,$entree_sortie){
 ...
 function pageGUI($form_name){
	...
	ligne('DOWNLOAD_PACK_DIR',$l->g(775),'radio',array('DEFAULT'=>"default (".$_SERVER["DOCUMENT_ROOT"]."/DOWNLOAD)",'CUSTOM'=>"Personnaliser",'VALUE'=>$select_pack),
		array('HIDDEN'=>'CUSTOM','HIDDEN_VALUE'=>$values['tvalue']['DOWNLOAD_PACK_DIR'],'SIZE'=>70,'END'=>"/DOWNLOAD"));
	ligne('IPDISCOVER_IPD_DIR',$l->g(776),'radio',array('DEFAULT'=>" default (".$_SERVER["DOCUMENT_ROOT"]."/oscreport/IPD)",'CUSTOM'=>"Personnaliser",'VALUE'=>$select_ipd),
...
function pagegroups($form_name){
 ...
 function pageserveur($form_name){
	...
	ligne('SECURITY_LEVEL',$l->g(739),'input',array('VALUE'=>check_param ($values['ivalue'], 'SECURITY_LEVEL'),'SIZE'=>1,'MAXLENGHT'=>3,'JAVASCRIPT'=>$numeric),'',"readonly");	
	...
	ligne('SESSION_VALIDITY_TIME',$l->g(777),'input',array('END'=>$l->g(511).$sup1,'VALUE'=>check_param ($values['ivalue'], 'SESSION_VALIDITY_TIME'),'SIZE'=>1,'MAXLENGHT'=>3,'JAVASCRIPT'=>$numeric));	
...
// 	}
	...
	$optvalueselected ='CUSTOM';
 	debut_tab(array('CELLSPACING'=>'5',
		...
		ligne('INVENTORY_SESSION_ONLY',$l->g(744),'radio',array(1=>'ON',0=>'OFF','VALUE'=>check_param ($values['ivalue'], 'INVENTORY_SESSION_ONLY')));
	fin_tab($form_name);
