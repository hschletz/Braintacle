...
if( isset($_GET["suppack"])) {
	...
	if( !isset($_GET["nonnot"]) )
		@mdb2_query("DELETE FROM download_enable WHERE ID=?", $_SESSION["writeServer"], "integer", $_GET["suppack"]) or die(mdb2_error());	
	
	$reqSupp = "DELETE FROM devices WHERE name='DOWNLOAD' AND ivalue=?";
	if( isset($_GET["nonnot"]) )
		$reqSupp .= " AND tvalue IS NULL";
	
	@mdb2_query($reqSupp, $_SESSION["writeServer"], "integer", $_GET["suppack"]) or die(mdb2_error());	
}
...
$from = "download_enable e JOIN download_available d ON d.fileid = e.fileid and e.FILEID=" . $_SESSION["readServer"]->quote ($_GET['timestamp'], "text") . ' AND e.id IS NOT NULL';
else
