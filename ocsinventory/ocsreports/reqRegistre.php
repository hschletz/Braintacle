...
switch (check_param ($_GET, "typeDemande")) :
	...
	default :
		...
		if(check_param ($_GET, "id"))
			mdb2_query("delete from regconfig where id=?", $_SESSION["writeServer"], "integer", $_GET["id"]) or die(mdb2_error($_SESSION["writeServer"]));
...
{	
	...
	echo   "<center><br><form name='ajouter_reg' method='POST' action='index.php?multi=5&typeDemande=$demande'>
		 <input type='hidden' name='ID' value='".htmlspecialchars(urldecode($id))."'>
		 <table>
		 <tr><td align='left'><font face=Verdana size=-1>".$l->g(252)." :</font> </td>
		 <td><input size=40 name='NAME' value='".htmlspecialchars(urldecode($name))."'></td></tr>
	...
	echo	 "			</select></td></tr>
		 ...
		 <td><input size=40 name='REGKEY' value='".htmlspecialchars(urldecode($regkey))."'> </td></tr>			 
		 <tr><td align='left'><font face=Verdana size=-1>".$l->g(255)." :</font>  </td>
		 <td><input size= 40 name='REGVALUE' value='".htmlspecialchars(urldecode($regvalue))."'></td></tr>
		 </table><br><br>
		 <table><tr><td>
...
function modifier()
{
	...
	if (check_param ($_GET, "id"))
	{
		$requete = "select ID,NAME,REGTREE,REGKEY,REGVALUE from regconfig where ID=?";
		$result  = mdb2_query($requete, $_SESSION["readServer"], "integer", $_GET["id"]) or die(mdb2_error($_SESSION["readServer"]));
	...
	{	
		...
		$result  = mdb2_query($requete, $_SESSION["readServer"]) or die(mdb2_error($_SESSION["readServer"]));
		...
		echo "</tr>";
			while($item = mdb2_fetch_object($result, CASE_UPPER))
			{
				echo "<tr><td align='center'><FONT FACE='Verdana' size=2><a href='index.php?multi=5&typeDemande=modif&id=$item->ID'>$item->ID&nbsp;&nbsp;&nbsp;</a></td> 
						  <td align='center'>" . htmlspecialchars($item->NAME) . "</td><td>" . htmlspecialchars($item->REGVALUE) . "</td></font></tr>";
			}
...
function enregistrer()
{
	...
	if($_GET["typeDemande"]== "enreModif") // enregistrer une modififcation
	{
		...
		$result = mdb2_query($req, $_SESSION["writeServer"], array ("text", "integer", "text", "text", "integer"), array ($_POST["NAME"], $_POST["REGTREE"], $_POST["REGKEY"], $_POST["REGVALUE"], $_POST["ID"])) or die(mdb2_error($_SESSION["writeServer"]));
	}	
	else // enregistrer un ajout
	{	
		if((isset($_POST["enre"]))) {
		$req    = "INSERT INTO regconfig (name,regtree,regkey,regvalue) VALUES(?,?,?,?)";
		$result = mdb2_query($req, $_SESSION["writeServer"], array ("text", "integer", "text", "text"), array ($_POST["NAME"], $_POST["REGTREE"], $_POST["REGKEY"], $_POST["REGVALUE"]));
		}
	}//fin else
	?>
...
function supprimer()
{
	...
	{	
		//echo   "<center><font face='Arial' color='#330033' size=4><b><center>OCS INVENTORY</center></b></font><hr>";
		echo   "<center>Select the registry entry you want to remove :<BR><BR><BR>";
