	...
	if($ok)
	{
		...
		{	
			if( $fname == "ocsagent.exe" || $fname == "ocspackage.exe" ) {
				@mdb2_query("DELETE FROM deploy WHERE name=?", $_SESSION["writeServer"], "text", $fname);
				$result = mdb2_query("INSERT INTO deploy (name, content) VALUES (?,?)", $_SESSION["writeServer"], array ("text", "blob"), array ($fname, $binary));
			} else {
				$result = mdb2_query("INSERT INTO files (name, version, os, content) VALUES (?,?,?,?)", $_SESSION["writeServer"], array ("text", "text", "text", "blob"), array ($fname, $version, $platform, $binary));
			}
			if($result)
			{
				echo "<br><b><center><font color='green'>".$l->g(137)." ".$_FILES['userfile']['name']." ".$l->g(234)."</font></center></b><br>";
			}
			else if(mdb2_errno()==MDB2_ERROR_ALREADY_EXISTS || mdb2_errno()==MDB2_ERROR_CONSTRAINT)
			{
			...
			else
			{
				...
				echo mdb2_error($_SESSION["writeServer"])."</font></center></b>";
			}
			sleep(2);
		}
	}	
...
if(check_param ($_GET, "o")&&$_GET["n"]&&$_GET["v"]&&$_GET["supp"]==1)
{
	if( strtolower($_GET["n"]) == "ocsagent.exe" ) {
		@mdb2_query("DELETE FROM deploy WHERE name='ocsagent.exe'");
	}
	else if( strtolower($_GET["n"]) == "ocspackage.exe" ) {
		@mdb2_query("DELETE FROM deploy WHERE name='ocspackage.exe'");
	}
	else
	{	
		$suppQuery="DELETE FROM files WHERE name=? AND os=? AND version=?";
		@mdb2_query($suppQuery, $_SESSION["writeServer"], array ("text", "text", "text"), array ($_GET["n"], $_GET["o"], $_GET["v"]));
	}
...
<?php 
	...
	if(!$resultD=mdb2_query($queryD, $_SESSION["readServer"]))
		echo mdb2_error($_SESSION["readServer"]);
	
	$query="SELECT version, os,name FROM files ORDER BY version DESC,os ASC,name ASC";
	
	if(!$result=mdb2_query($query, $_SESSION["readServer"]))
		echo mdb2_error($_SESSION["readServer"]);
	$x=0;
	
	$item = mdb2_fetch_object($resultD);
	if( $item == NULL or $item->name =="")
		$item=mdb2_fetch_object($result);
	...
	?>		
		...
		<td align=center><a href=javascript:void(0); OnClick='window.open("download.php?dl=1&v=<?php echo $item->version?>&o=<?php echo $item->os?>&n=<?php echo strtolower($item->name) ?>","fen","location=0,status=0,scrollbars=0,menubar=0,resizable=0,width=1,height=1")'><img src=image/Gest_admin1.png></a></FONT></td>
	...
	<?php }
	while($item=mdb2_fetch_object($resultD));
?>
<table><br>
