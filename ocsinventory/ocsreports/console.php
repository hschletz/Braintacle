...
check_param ($_POST, "tri2", "^[0-9a-zA-Z]*$");
...
//function for only count before show result
 function query_on_table_count($name,$lbl_data,$tablename="hardware"){
 	...
 	if (!isset($list_no_show[$name])){
	 	$sql_on_hardware="SELECT COUNT($name) AS c
						from ".$tablename." h ";
		...
		$sql_on_hardware.=" having COUNT($name) != 0 ";
	 	$result_on_hardware = mdb2_query( $sql_on_hardware, $_SESSION["readServer"]);
	 	$num_rows = mdb2_num_rows($result_on_hardware);
...
//function for show all result  
 function query_on_table($name,$lbl_data,$lbl_data_detail,$tablename="hardware"){
	...
	$name_quoted = mdb2_quote_identifier ($name, $_SESSION["readServer"]);
 	if (!isset($list_no_show[$name])){
	 	...
	 	$sql_on_hardware = "SELECT COUNT($name_quoted) AS c, $name_quoted AS name
						from " . mdb2_quote_identifier ($tablename, $_SESSION["readServer"]) . " h ";
		if ($tablename=="hardware"){
			if ($list_on_hardware == "")
				$sql_on_hardware.=" where ".$exlu_group;
			else
		...
		$sql_on_hardware.=$list_on_else;
		$sql_on_hardware.="	group by ".$name_quoted;
		$_SESSION["forcedRequest"]=$sql_on_hardware;
		$sql_on_hardware .= " order by $_POST[tri2] $_POST[sens] limit $limit[END] offset $limit[BEGIN]";
	 	$result_on_hardware = mdb2_query( $sql_on_hardware, $_SESSION["readServer"]);
		$nb_lign=0;
		while($item_on_hardware = mdb2_fetch_object($result_on_hardware)){
 ...
 function query_with_condition($wherecondition,$lbl_data,$name_data,$tablename="hardware",$link=""){
 	...
 	if (!isset($list_no_show[$name_data])){
	 	$sql_count="SELECT COUNT(*) AS c FROM $tablename h ";
	 	...
	 	$result_count = mdb2_query( $sql_count, $_SESSION["readServer"]);
		$item_count = mdb2_fetch_object($result_count);
...
if( $_SESSION["lvluser"] == SADMIN) {
	...
	$result_frequency = mdb2_query( $sql_frequency, $_SESSION["readServer"]);
	$item_frequency = mdb2_fetch_object($result_frequency);
	
	if (isset($_POST['supp']) and $_POST['supp'] != ""){
		$sql_not_show = "DELETE FROM config WHERE name=?";
		mdb2_query( $sql_not_show, $_SESSION["writeServer"], "text", $_POST["supp"]);
		
	}	
	
	 if (isset($_POST['DELETE_OPTION']) and $_POST['DELETE_OPTION'] != ""){
			mdb2_query ("DELETE FROM config WHERE name=?", $_SESSION["writeServer"], "text", "OSC_REPORT_$_POST[DELETE_OPTION]");
			$sql_not_show = "INSERT INTO config (name,ivalue) VALUES (?,1)";
			mdb2_query ($sql_not_show, $_SESSION["writeServer"], "text", "OSC_REPORT_$_POST[DELETE_OPTION]");

	 }
	 if (isset($_POST['USE_OPTION']) and $_POST['USE_OPTION'] != ""){
			$sql_show = "DELETE FROM config WHERE name=?";
			mdb2_query ($sql_show, $_SESSION["writeServer"], "text", "OSC_REPORT_$_POST[USE_OPTION]");
	...
	if (isset($_POST['Valid']) and $_POST['onglet'] == "CONFIG"){
		foreach ($_POST as $key=>$value){
			...
			if ($value != "" and $key != "Valid" and $key != 'onglet' and $key !='pcparpage'){
				...
				if ($key == 'LAST_DIFF' and $value < $item_frequency->ivalue)
					echo "<script> alert('Value of LAST_DIFF must be greater than FREQUENCY')</script>";
				else{
					$sql = "DELETE FROM config WHERE name=?";
					mdb2_query ($sql, $_SESSION["writeServer"], "text", "GUI_REPORT_$key");
					$sql="INSERT INTO config (name,ivalue) VALUES (?,?)";
					mdb2_query ($sql, $_SESSION["writeServer"], array ("text", "integer"), array ("GUI_REPORT_$key", $value));
				}
				
			}
			
		}
	}elseif (check_param ($_POST, "onglet") == "MSG" and isset($_POST['Val'])){
		$sql_msg="SELECT name FROM config WHERE name $LIKE 'GUI_REPORT_MSG%'";
		$result_msg = mdb2_query( $sql_msg, $_SESSION["readServer"]);
		while($item_msg = mdb2_fetch_object($result_msg)){
		...
		$i=1;
		if (trim(check_param ($_POST, 'GROUP')) != "" and is_numeric($_POST['GROUP']) and trim($_POST['MSG'])!=""){
			$sql = "INSERT INTO config (name,ivalue,tvalue) VALUES (?,?,?)";
			mdb2_query ($sql, $_SESSION["writeServer"], array ("text", "integer", "text"), array ("GUI_REPORT_MSG$i", $_POST["GROUP"], $_POST["MSG"]));
		}else
 ...
 $sql_search_option = "SELECT " . $_SESSION["readServer"]->function->substring ("name", 12) . " AS name FROM config WHERE name $LIKE 'OSC_REPORT_%'";
 $result_search_option = mdb2_query( $sql_search_option, $_SESSION["readServer"]);
while($item_search_option = mdb2_fetch_object($result_search_option, CASE_UPPER))
	$list_no_show[$item_search_option ->NAME]=$item_search_option ->NAME;	

//witch option fields
 $sql_search_option = "SELECT " . $_SESSION["readServer"]->function->substring ("name", 12) . " AS name, ivalue FROM config WHERE name $LIKE 'GUI_REPORT_%'";
 $result_search_option = mdb2_query( $sql_search_option, $_SESSION["readServer"]);
while($item_search_option = mdb2_fetch_object($result_search_option, CASE_UPPER))
...
if (isset($default)){
	...
	if( $_SESSION["lvluser"] == ADMIN) {
		$sql_hardware_id= "SELECT hardware_id AS id FROM accountinfo a WHERE ".$_SESSION["mesmachines"];
		$result_hardware_id = mdb2_query( $sql_hardware_id, $_SESSION["readServer"]);
		$nb_computor=0;
		while($item_hardware_id = mdb2_fetch_object($result_hardware_id)){
			$nb_computor++;
	...
	if ($_POST['onglet'] == "ACTIVITY"){
		...
		if (!isset($list_no_show['NB_ALL_COMPUTOR']) and !isset($list_no_show['NB_COMPUTOR'])){
			$sql_count_computer = "SELECT COUNT(*) AS c FROM hardware h 
								 where ".$exlu_group;
			$result_count_computer = mdb2_query( $sql_count_computer, $_SESSION["readServer"]);
			$item_count_computer = mdb2_fetch_object($result_count_computer);
	 	...
	 	query_with_condition ("WHERE lastcome > CURRENT_DATE ",
							 $lbl_field['NB_CONTACT'],'NB_CONTACT');
		query_with_condition ("WHERE lastdate > CURRENT_DATE ",
...
//							 $lbl_field['NB_4_MOMENT']." ".$list_option['AGIN_MACH']." ".$l->g(496),'NB_4_MOMENT');
		query_with_condition ("WHERE " . $_SESSION["readServer"]->function->unixtimestamp ("lastdate") . " < (" . $_SESSION["readServer"]->function->unixtimestamp("CURRENT_TIMESTAMP") . " - $list_option[AGIN_MACH] * 86400) ",
							 $lbl_field['NB_4_MOMENT']." ".$list_option['AGIN_MACH']." ".$l->g(496),'NB_4_MOMENT');
		if (isset($list_option['LAST_DIFF'])){
				query_with_condition ("WHERE FLOOR ((" . $_SESSION["readServer"]->function->unixtimestamp ("lastcome") . " - " . $_SESSION["readServer"]->function->unixtimestamp ("lastdate") . " ) / 86400) >= $list_option[LAST_DIFF]",
	...
	if ($_POST['onglet'] == "ELSE"){
		...
		query_with_condition(" where  h.name='DOWNLOAD' and " . $_SESSION["readServer"]->function->substring ("h.tvalue",1,3) . "='ERR'",$lbl_field['NB_ERR'],'NB_ERR',"devices");
	}
	...
	if ($_POST['onglet'] == "CONFIG"){
		...
		if (isset ($list_no_show_cat)){
			$list_no_show_cat['']="";
			ksort($list_no_show_cat);
	 		ligne('USE_OPTION',$l->g(802),'select',array('VALUE'=>check_param ($_POST, "USE_OPTION"),'SELECT_VALUE'=>$list_no_show_cat,'RELOAD'=>$form_name));
		}
	 	...
	 	$text="<br><font color=orange><i>Value must be greater than " . $item_frequency->ivalue . " day(s) (value of FREQUENCY)</i></font>";
	 	ligne('LAST_DIFF','Difference between LASTDATE and LASTCOME','input',array('VALUE'=>check_param ($list_option, "LAST_DIFF"),'END'=>$l->g(496).$text,'SIZE'=>2,'MAXLENGHT'=>3,'JAVASCRIPT'=>$numeric));
	 	}
	...
	if ($_POST['onglet'] == "MSG"){
		...
		$entete[]=$l->g(392);
		//print_r($entete);
		$sql_msg = "SELECT h.name AS hname, c.name AS cname, c.ivalue, c.tvalue FROM config c,hardware h
				 where h.id=c.ivalue
		...
		while($item_msg = mdb2_fetch_object($result_msg)){
			...
			$data_msg[$i]['tvalue']=$item_msg ->tvalue;
		...
		if (check_param ($_POST, "add_text")){
			...
			$result_group_list = mdb2_query( $sql_group_list, $_SESSION["readServer"]);
			$list_group['']='';
			while($item_group_list = mdb2_fetch_object($result_group_list, CASE_UPPER)){
	...
	if (isset($data)){
		 foreach ($data as $key=>$value){
		 	echo "<tr height=30px bgcolor='#F2F2F2' BORDERCOLOR='#9894B5'>
				<td align='center' width='300px'><font size=2>" . htmlspecialchars ($value["lbl"]) . "</font></td>
				<td align='center' width='150px'><B>".$value["data"]."</B></td></tr>";
		 	
		 }
	}
	 echo "</table></table>";
	echo "<input type='hidden' id='detail' name='detail' value='" . check_param ($_POST, "detail") . "'>";
	echo "<input type='hidden' id='tablename' name='tablename' value='" . check_param ($_POST, "tablename") . "'>";
	...
	if (check_param ($_POST, "detail") != "" 
		...
		if (check_param ($_POST, "sens") == "ASC")
			...
			$sens="ASC";
		$deb="<a OnClick='tri(\"\",\"".$sens."\",\"".$form_name."\")' >";
		$fin="</a>";
		...
		show_page($data['nb_'.$_POST['detail']]['count'],$form_name);
			...
			if (check_param ($_POST, 'tri2') == "" or !isset($FIELDS[$_POST['tri2']]))
				$_POST['tri2']=1;
			$limit=nb_page($form_name);
			$trans = array("COUNT(*) AS c" => implode(",", $FIELDS)); // Hope that the other query, wherever it is built, matches EXACTLY.
			$sql= strtr($_SESSION['SQL'][$_POST['detail']], $trans); // This is asking for trouble!
			$_SESSION["forcedRequest"]=$sql;
			$sql.= " order by ".$_POST['tri2']." ".check_param ($_POST, 'sens');
			$sql.= " LIMIT $limit[END] OFFSET $limit[BEGIN]";
			$resCount = mdb2_query($_SESSION['SQL'][$_POST['detail']], $_SESSION["readServer"]) 
				or die(mdb2_error($_SESSION["readServer"]));
			...
			while($item = mdb2_fetch_object($result, CASE_UPPER)){
				...
				foreach ($FIELDS as $key=>$value){					
					$key = strtoupper ($key);
					if ($key != 'ID'){
						if (check_param ($FIELDS_LINK, $key))
							$data[$i][check_param ($entete, $j)] = $deb . htmlspecialchars ($item->$key) . $fin;
						else
							$data[$i][check_param ($entete, $j)] = htmlspecialchars ($item->$key);
					}		
					$j++;	
				}
				$i++;
...
if ($_SESSION["lvluser"] == ADMIN){
	...
	$result_all_msg = mdb2_query ($sql_all_msg, $_SESSION["readServer"]);
	$list_id_groups="";
	while($item_all_msg = mdb2_fetch_object($result_all_msg)){
	...
	if ($list_id_groups != ""){
		$sql_my_msg="SELECT DISTINCT g_c.group_id AS groups 
		...
		$result_my_msg = mdb2_query( $sql_my_msg, $_SESSION["readServer"]);
		echo "<table align=center><tr><td align=center>";
		while($item_my_msg = mdb2_fetch_object($result_my_msg)){
			$i=0;
			while ($list_all_msg[$item_my_msg ->groups]['TVALUE'][$i]){
				echo "<font color=red size=4>" . htmlspecialchars ($list_all_msg[$item_my_msg ->groups]['TVALUE'][$i]) . "</font></td></tr><tr><td align=center>";		
				$i++;
			}
		}	
		echo "</td></tr></table>";
