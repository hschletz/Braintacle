...
if(isset($_POST["actpack"]) and check_param ($_POST, 'activat_option') == "for_server")
{
	if ($_POST['id_server_add'] != ""){
		//search for machines which already have the package assigned
		$sqlDoub="select SERVER_ID,INFO_LOC from download_enable where FILEID=?";
		$resDoub = mdb2_query( $sqlDoub, $_SESSION["readServer"], "text", $_POST["actpack"]);	
		$listDoub="";
		//create exclusion list
		while ($valDoub = mdb2_fetch_assoc( $resDoub, CASE_UPPER )){
			if ($valDoub['SERVER_ID'] != "")
			$listDoub.=$valDoub['SERVER_ID'].",";	

			//Update https server location if different from database
                        if ($valDoub['INFO_LOC'] != $_POST['https_server']) {
                                $sql_update_https= "UPDATE download_enable SET download_enable.INFO_LOC=? WHERE SERVER_ID=?";
                                mdb2_query( $sql_update_https, $_SESSION["readServer"], array('text', 'integer'), array($_POST['https_server'], $valDoub['SERVER_ID']) );
                        }
		}
		//if the list is not empty, create the missing part of the request
		if ($listDoub != ""){
		...
		$sql="insert into download_enable (FILEID,INFO_LOC,PACK_LOC,CERT_PATH,CERT_FILE,SERVER_ID,GROUP_ID)
			 ...
			 where GROUP_ID=? $listDoub";
		mdb2_query( $sql, $_SESSION["writeServer"], array ("text", "text", "integer"), array ($_POST['actpack'], $_POST['https_server'], $_POST['id_server_add'])) or die(mdb2_error($_SESSION["writeServer"]));
	}
...
if( isset( $_POST["actpack"] ) and check_param ($_POST, 'activat_option') != "for_server") {
	...
	$reqFrags = "SELECT fragments FROM download_available WHERE fileid=?";
	$resFrags = mdb2_query( $reqFrags, $_SESSION["readServer"], "text", $_POST["actpack"]);	
	$valFrags = mdb2_fetch_assoc( $resFrags );
	...
	<?php 
		...
		$reqVerif = "SELECT * FROM download_available WHERE fileid=?";
		if( ! mdb2_num_rows( mdb2_query( $reqVerif, $_SESSION["readServer"], "text", $_POST["actpack"]) )) {
			...
			mdb2_query( $req1, $_SESSION["writeServer"], array ("text", "text", "integer", "text"), array ($_POST["actpack"], "Manual_$_POST[actpack]", $infoTab["PRI"], $infoTab["FRAGS"]));
		}
		
		$req = "INSERT INTO download_enable(FILEID, INFO_LOC, PACK_LOC, CERT_FILE, CERT_PATH ) VALUES
		( ?,?,?, 'INSTALL_PATH/cacert.pem','INSTALL_PATH')";
	
		mdb2_query( $req, $_SESSION["writeServer"], array ("text", "text", "text"), array ($_POST["actpack"], $_POST["https"], $_POST["frag"]));
	...
	$reqEnable = "SELECT id FROM download_enable WHERE FILEID=?";
	$resEnable = @mdb2_query($reqEnable, $_SESSION["writeServer"], "text", $_GET["suppack"]) or die(mdb2_error());
	
	while($valEnable = mdb2_fetch_assoc( $resEnable ) ) {
		$reqDelDevices = "DELETE FROM devices WHERE name='DOWNLOAD' AND ivalue=".$valEnable["id"];
		@mdb2_query($reqDelDevices, $_SESSION["writeServer"]) or die(mdb2_error());
	}
	$reqDelEnable = "DELETE FROM download_enable WHERE FILEID=?";
	@mdb2_query($reqDelEnable, $_SESSION["writeServer"], "text", $_GET["suppack"]) or die(mdb2_error());
	$reqDelAvailable = "DELETE FROM download_available WHERE FILEID=?";
	@mdb2_query($reqDelAvailable, $_SESSION["writeServer"], "text", $_GET["suppack"]) or die(mdb2_error());
	//looking for the directory for pack
	$sql_document_root="select tvalue from config where NAME='DOWNLOAD_PACK_DIR'";
	$res_document_root = mdb2_query( $sql_document_root, $_SESSION["readServer"] );
	while( $val_document_root = mdb2_fetch_assoc( $res_document_root ) ) {
	...
	if( ! @recursive_remove_directory( $document_root."/download/" . check_param ($_GET, "suppack", "^[0-9]+$") ))  {
...
function loadInfo( $serv, $tstamp ) {
	...
	$fname = "https://" . rawurlencode ($serv) . "/" . rawurlencode ($tstamp) . "/info";
