...
<?php
$result = mdb2_query("select distinct rule,rule_name from download_affect_rules", $_SESSION["readServer"]) or die(mdb2_error($_SESSION["readServer"]));
$list = "";
while($item = mdb2_fetch_object($result))
	$list.="<option value='" . htmlspecialchars ($item->rule) . "'>" . htmlspecialchars ($item->rule_name) . "</option>";	
