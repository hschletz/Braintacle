...
allowing centralized software updates, remote configuration and more.


## Requirements

- A [PostgreSQL](https://postgresql.org) or [MySQL](https://www.mysql.com) (or
  one of its derivates, like [MariaDB](https://mariadb.org)) database. Support
  for other database backends may be added in the future.

- [Perl](https://perl.org) and some Perl modules (see installation instructions),

- A web server, either [Apache httpd](http://httpd.apache.org) with
  [mod_perl](https://perl.apache.org) or any web server capabale of executing
  CGI scripts, for the main server component.
...
- On every client that should be managed through Braintacle, the [OCS Inventory
  NG agent](https://github.com/OCSInventory-NG/) must be installed.
