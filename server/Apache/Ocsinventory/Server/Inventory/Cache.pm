...
sub _cache{
  ...
  for my $field ( @fields_array ){
    ...
    if( $err && $err == 0E0 && $op eq 'add'){
      ...
      my $err2 = $dbh->do("SELECT $field FROM $section WHERE $field=? LIMIT 1", {}, $values->[ $sectionMeta->{field_cached}->{$field} ]);
      if( $err2 && $err2 == 0E0 ){
...
sub _reset_inventory_cache{
  ...
  if( &_check_cache_validity() ){
    ...
    if( &_lock_cache() ){
      for my $section ( @$sectionsList ){
        ...
        for my $field (@fields_array){
          ...
          } else { # alternative implementation for non-MySQL databases
            # Table locking ist not portable across different DBMS.
            # The best way to handle concurrent transactions in this case is to make it serializable.
            # This may cause a transaction to fail in case of concurrent writes.
            # If that happens we just give up.
            # This implementation is far from perfect, as is the whole idea of duplicating data within the database.
            # It is strongly recommended to turn off caching alltogether.
            $dbh->commit; # start a new transaction
    ...
    $dbh->do('INSERT INTO engine_persistent(NAME,IVALUE) VALUES(\'INVENTORY_CACHE_CLEAN_DATE\', ' . compose_unix_timestamp('CURRENT_TIMESTAMP'). ')')
      if($dbh->do('UPDATE engine_persistent SET IVALUE=' . compose_unix_timestamp('CURRENT_TIMESTAMP') . ' WHERE NAME=\'INVENTORY_CACHE_CLEAN_DATE\'')==0E0);
...
sub _check_cache_validity{
  my $dbh = $Apache::Ocsinventory::CURRENT_CONTEXT{'DBI_HANDLE'};
  my $check_cache;
  $check_cache = $dbh->prepare('SELECT ' . compose_unix_timestamp('CURRENT_TIMESTAMP') . '-IVALUE AS IVALUE FROM engine_persistent WHERE NAME=\'INVENTORY_CACHE_CLEAN_DATE\'');
  $check_cache->execute();
  if($check_cache->rows()){
