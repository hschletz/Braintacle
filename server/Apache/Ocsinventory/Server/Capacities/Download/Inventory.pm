...
sub update_history_full{
  ...
  my $sth = $dbh->prepare('INSERT INTO download_history(HARDWARE_ID, PKG_ID) VALUES(?,?)');
...
sub update_history_diff{
  ...
  for my $l_xml (@$fromXml){
    ...
    if(!$found){
      $dbh->do( 'INSERT INTO download_history(HARDWARE_ID, PKG_ID) VALUES(?,?)', {}, $hardwareId, $l_xml )
