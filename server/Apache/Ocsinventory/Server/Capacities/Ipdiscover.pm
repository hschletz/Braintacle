...
sub _ipdiscover_read_result{
  ...
  if(exists($result->{CONTENT}->{IPDISCOVER})){
    ...
    if( $ENV{'OCS_OPT_IPDISCOVER_LINK_TAG_NETWORK'} ) {
      ...
      for(@$base){
        ...
        unless($update_req->rows){
          ...
          $insert_req->execute($_->{I}, uc($_->{M}), $mask, $subnet, $_->{N}, $tag);
        }
      }
    } else {
    # We insert the results (MAC/IP)
    $update_req = $dbh->prepare('UPDATE netmap SET IP=?,MASK=?,NETID=?,DATE=CURRENT_TIMESTAMP, NAME=? WHERE MAC=?');
    $insert_req = $dbh->prepare('INSERT INTO netmap(IP, MAC, MASK, NETID, DATE, NAME) VALUES(?,?,?,?,CURRENT_TIMESTAMP,?)');
    ...
    for(@$base){
      ...
      $update_req->execute($_->{I}, $mask, $subnet, $_->{N}, uc($_->{M}));
      unless($update_req->rows){
        $insert_req->execute($_->{I}, uc($_->{M}), $mask, $subnet, $_->{N});
      }
    }
    }
...
sub _ipdiscover_evaluate{
  ...
  for(@$base){
    if(defined($_->{IPSUBNET}) and $_->{IPSUBNET}=~/^(\d{1,3}(?:\.\d{1,3}){3})$/ ){
      ...
      $request = $dbh->prepare('
      SELECT h.ID AS ID, h.QUALITY AS QUALITY, ' . compose_unix_timestamp('h.LASTDATE') . ' AS LAST 
